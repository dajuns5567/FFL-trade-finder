...
window.coveredPrecisionWithV384FallbackAudit=function(){
 const arr=window.ensureMaster?.()||[],gate=window.offenseConsensusCoverageGateV384,num=v=>{const n=Number(v);return Number.isFinite(n)?n:null};
 const terminal=[['rbCalibrationV49','offenseTerminalExactV49'],['rbCalibrationV48','offenseTerminalExactV48'],['youngCalibrationV47','offenseTerminalExactV47'],['youngIdentityV45','offenseTerminalExactV45'],['offenseContextV43','offenseTerminalExactV43']];
 const rows=arr.map((z,i)=>{const p=z.production||{},pos=window.groupPos?.(z.x),served=num(z.value);let candidate=served,source='served';
   if(pos==='IDP'){const b=num(p.idpOverallTradeCurveBaselineV72),fac=num(p.idpOverallTradeCurveFactorV72);if(b!=null&&fac!=null){candidate=b*fac;source='IDP-V72';}}
   else{let covered=null;try{covered=gate?.offenseCoverage?.(z.x?.id)}catch(_){}
     if(covered===false||z.coverageGate384){candidate=served;source='V384-uncovered-served-fallback';}
     else{for(const [flag,key] of terminal){if(p[flag]){const e=num(p[key]);if(e!=null){candidate=e;source='OFF-covered-terminal-exact';}break;}}}
   }
   return{id:String(z.x?.id??''),name:window.playerName?.(z.x?.id)||String(z.x?.id??''),pos,served,candidate,source,oldRank:i+1,gateUncovered:!!z.coverageGate384};
 });
 const cand=[...rows].sort((a,b)=>b.candidate-a.candidate||a.oldRank-b.oldRank);cand.forEach((r,i)=>{r.newRank=i+1;r.delta=r.oldRank-r.newRank});
 const tieStats=(key)=>{const m=new Map();for(const r of rows)m.set(r[key],(m.get(r[key])||0)+1);const g=[...m.values()].filter(n=>n>1);return{groups:g.length,players:g.reduce((a,n)=>a+n,0),max:g.length?Math.max(...g):1,distinct:m.size}};
 const sources={};for(const r of rows)sources[r.source]=(sources[r.source]||0)+1;
 ...
 const movers=cand.filter(r=>r.delta).sort((a,b)=>Math.abs(b.delta)-Math.abs(a.delta)).slice(0,50);
 const uncovered=cand.filter(r=>r.gateUncovered).map(r=>({id:r.id,name:r.name,served:r.served,candidate:r.candidate,oldRank:r.oldRank,newRank:r.newRank,delta:r.delta})).slice(0,40);
 return{criterion:'diagnostic only: preserve exact terminal precision for covered offense, preserve V384 served fallback values for all uncovered offense, combine with V72 precision; no runtime mutation',summary:{players:rows.length,sources,served:tieStats('served'),candidate:tieStats('candidate')},cutoffs,uncoveredCheck:uncovered,largestMovers:movers};
};

window.fullPrecisionV72CoverageMarketAudit=function(){
 const arr=window.ensureMaster?.()||[],gate=window.offenseConsensusCoverageGateV384,num=v=>{const n=Number(v);return Number.isFinite(n)?n:null};
 const terminal=[['rbCalibrationV49','offenseTerminalExactV49'],['rbCalibrationV48','offenseTerminalExactV48'],['youngCalibrationV47','offenseTerminalExactV47'],['youngIdentityV45','offenseTerminalExactV45'],['offenseContextV43','offenseTerminalExactV43']];
 const rows=arr.map((z,i)=>{const p=z.production||{},pos=window.groupPos?.(z.x),served=num(z.value);let candidate=served,source='served';
   if(pos==='IDP'){const e=num(p.idpOverallTradeCurveExactV72);if(e!=null){candidate=e;source='IDP-V72-full-exact';}}
   else{let covered=null;try{covered=gate?.offenseCoverage?.(z.x?.id)}catch(_){}
     if(covered===false||z.coverageGate384){candidate=served;source='V384-uncovered-served-fallback';}
     else{for(const [flag,key] of terminal){if(p[flag]){const e=num(p[key]);if(e!=null){candidate=e;source='OFF-covered-terminal-exact';}break;}}}
   }
 ...
 const cand=[...rows].sort((a,b)=>b.candidate-a.candidate||a.oldRank-b.oldRank);cand.forEach((r,i)=>{r.newRank=i+1;r.delta=r.oldRank-r.newRank});
 const stats=k=>{const m=new Map();for(const r of rows)m.set(r[k],(m.get(r[k])||0)+1);const g=[...m.values()].filter(n=>n>1);return{groups:g.length,players:g.reduce((a,n)=>a+n,0),max:g.length?Math.max(...g):1,distinct:m.size}};
 const sources={};for(const r of rows)sources[r.source]=(sources[r.source]||0)+1;
 const cutoffs=[50,100,150,200,300,400,500].map(n=>{const g=cand.slice(0,n),idp=g.filter(r=>r.pos==='IDP').length;return{top:n,idp,offense:n-idp,cutoff:g.at(-1)?.candidate??null}});
...
window.exactTieCanonicalRegressionAudit=function(){
 ...
 for(let i=0;i<arr.length;){const mv=z=>{const p=Number(z?.marketPrecisionValueV386),x=Number(z?.value);return Number.isFinite(p)&&p>0?p:(Number.isFinite(x)&&x>0?x:0)},v=mv(arr[i]);let j=i+1;while(j<arr.length&&mv(arr[j])===v)j++;if(j-i>1){const rows=arr.slice(i,j).map((z,k)=>({rank:i+k+1,name:window.playerName?.(z.x?.id)||String(z.x?.id??''),precision:v,canonical:Number(snap.get(String(z.x?.id??'')))})),bandEnds=[12,24,48,80,120,180,260],bands=new Set(rows.map(r=>bandEnds.find(e=>r.rank<=e)||arr.length));groups.push({precision:v,count:rows.length,crossesBandBoundary:bands.size>1,canonicalDistinct:[...new Set(rows.map(r=>r.canonical))],rows})}i=j}
...
window.precisionFinalRegressionAudit=function(){
 const arr=window.ensureMaster?.()||[],api=window.modeledPlayerValuesV319;api?.build?.(true);const snap=api?.snapshot?.()||new Map(),num=v=>{const n=Number(v);return Number.isFinite(n)?n:null};
 const rows=arr.map((z,i)=>({id:String(z?.x?.id??''),name:window.playerName?.(z?.x?.id)||String(z?.x?.id??''),group:window.groupPos?.(z.x)||null,rank:i+1,priorRank:num(z.marketPrecisionPriorRankV386),served:num(z.value),precision:num(z.marketPrecisionValueV386),source:z.marketPrecisionSourceV386||null,canonical:num(snap.get(String(z?.x?.id??''))),gate:!!z.coverageGate384}));
 const density=[50,100,150,200,300,400,500].map(n=>({top:n,idp:rows.slice(0,n).filter(r=>r.group==='IDP').length,offense:rows.slice(0,n).filter(r=>r.group!=='IDP').length}));
 const sourceCounts={};for(const r of rows)sourceCounts[r.source]=(sourceCounts[r.source]||0)+1;
 const precisionMonotonic=rows.every((r,i)=>i===0||r.precision<=rows[i-1].precision),canonicalMonotonic=rows.every((r,i)=>i===0||r.canonical<=rows[i-1].canonical),v384=rows.filter(r=>r.gate),v384FallbackPrecisionEqualsServed=v384.every(r=>r.precision===r.served);
 let sameBandTieViolations=0,tieGroups=0;const ends=[12,24,48,80,120,180,260],band=r=>ends.find(e=>r<=e)||rows.length;
 for(let i=0;i<rows.length;){let j=i+1;while(j<rows.length&&rows[j].precision===rows[i].precision)j++;if(j-i>1){tieGroups++;const g=rows.slice(i,j);if(new Set(g.map(r=>band(r.rank))).size===1&&new Set(g.map(r=>r.canonical)).size>1)sameBandTieViolations++}i=j}
 const keyNames=['Jeffery Simmons','Jeremiah Owusu-Koramoah','Edgerrin Cooper','Nick Bosa','Maxx Crosby','T.J. Watt','Myles Garrett','Will Anderson','Aidan Hutchinson','Tuli Tuipulotu','Nik Bonitto','Micah Parsons','Jared Verse','Dallas Turner','Laiatu Latu','Jalen Carter','James Pearce'];
 const norm=s=>String(s||'').toLowerCase().replace(/[^a-z0-9]/g,'');const wanted=new Set(keyNames.map(norm));const keyPlayers=rows.filter(r=>wanted.has(norm(r.name))).map(r=>({...r,rankDelta:r.priorRank==null?null:r.priorRank-r.rank}));
 const largestMovers=rows.map(r=>({...r,rankDelta:r.priorRank==null?0:r.priorRank-r.rank})).filter(r=>r.rankDelta).sort((a,b)=>Math.abs(b.rankDelta)-Math.abs(a.rankDelta)).slice(0,30);
 return{criterion:'final PR runtime regression: full-precision combined ranking + V384 fallback + V319 macro canonical mapping + exact-tie equalization; diagnostic only',summary:{players:rows.length,sourceCounts,precisionMonotonic,canonicalMonotonic,v384Rows:v384.length,v384FallbackPrecisionEqualsServed,tieGroups,sameBandTieViolations},density,keyPlayers,largestMovers};
};
