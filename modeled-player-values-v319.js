...
window.directCanonicalValueAudit=function(){
  ...
  build(true);
  const rows=arr.map((z,i)=>{const id=String(z?.x?.id??''),model=Number(z?.value),canonical=Number(map.get(id));return{id,name:window.playerName?.(id)||id,group:window.groupPos?.(z.x)||null,overallRank:i+1,modelValue:model,currentCanonicalValue:canonical,directCanonicalCandidate:model,difference:Number.isFinite(model)&&Number.isFinite(canonical)?canonical-model:null,ratio:Number.isFinite(model)&&model>0&&Number.isFinite(canonical)?+(canonical/model).toFixed(4):null}}).filter(r=>r.id&&Number.isFinite(r.modelValue));
  ...
  return{criterion:'diagnostic only: quantify distortion introduced after final calculated player values by V319 rank-based canonical remapping; direct candidate equals current final calculated model value; no runtime mutation',mapping:{bandEnds:[...BAND_ENDS],blend:BLEND,minRatio:MIN_RATIO,maxRatio:MAX_RATIO,forcedTopValue:MAX},summary:{all:summary(rows),offense:summary(rows.filter(r=>['QB','RB','WR','TE'].includes(r.group))),IDP:summary(rows.filter(r=>r.group==='IDP'))},bands,rankControls:controls,largestAbsoluteDistortions:topAbs,orderMismatch};
};
