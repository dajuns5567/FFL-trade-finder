...
function outlookStory(t,w,voice,v){
 ...
 if(a.fantasy_season_complete||Number(w)>=17){
  ...
  const rows={
   nick:[t.team_name+' has '+issue+' to sort before the next lineup locks.',t.team_name+' has a depth-chart problem waiting next week: '+issue+'. Depth gets less theoretical now.',t.team_name+' carries '+issue+' into the next Sunday, so the bench is about to earn its rent.',t.team_name+' finds '+issue+' on the front-office desk. There is the week’s first homework assignment.'],
   bart:[t.team_name+' arrives at the next engagement with '+issue+', an inelegant accessory but a manageable one.',t.team_name+' gets an inelegant next-week complication: '+issue+'. Depth, please report to the dining room.',t.team_name+' must account for '+issue+' before lineup lock. Nothing ruins presentation like missing personnel.',t.team_name+' has some unpleasant housekeeping ahead: '+issue+'. I expect competence and decent lighting.'],
   tilly:[t.team_name+' has '+issue+' staring at the next lineup. Somebody wake the depth chart.',t.team_name+' can see the next roster mess coming: '+issue+'. The emergency font has been notified.',t.team_name+' brings '+issue+' into next week, which is exactly how calm Tuesdays become stupid Sundays.',t.team_name+' has its next roster problem already labeled: '+issue+'. We have preheated the panic index responsibly.'],
   jeff:[t.team_name+' has '+issue+' entered into the next-week personnel file.',t.team_name+' has a new item on the docket: '+issue+'. Depth will now be called as a witness.',t.team_name+' carries '+issue+' into the next lineup investigation.',t.team_name+' already has '+issue+' in the next-week file. Management has advance notice and therefore fewer excuses.']
  };personnel=pick(rows[voice],v);
 }else{
  const rows={
