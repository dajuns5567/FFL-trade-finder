...
function upset(rows){
  ...
  for(const t of rows){const oid=String(t.next_opponent_roster_id||'');if(!oid)continue;const o=byId.get(oid);if(!o)continue;const key=[String(t.roster_id),oid].sort().join('|');if(seen.has(key))continue;seen.add(key);const a=rank(t)>rank(o)?t:o,b=a===t?o:t;const edge=recent(a)-recent(b)+(strength(a)-strength(b))*0.15;candidates.push({underdog:a,favorite:b,edge})}
...
export function buildHumanLeagueOverview({season,week,teams,players,transactions,canonicalTrades=[],weekClassification,reporters,valueHistoryMeta={}}){
  ...
  const sections=[
    {reporter:reporterPublic(reporters[0]),heading:'The League Woke Up Talking About This',paragraphs:[
      top?top.team_name+' owned the loudest scoreboard of Week '+week+' with '+one(top.points)+' points, and nobody needed a spreadsheet to notice. '+(margin&&margin.roster_id!==top.roster_id?margin.team_name+' supplied the other result people kept checking, a '+one(Math.abs(Number(margin.points)-Number(margin.opponent_points)))+'-point blowout.':'It was the kind of performance that gets screenshotted before the app has even finished updating.')+' By Sunday night, half the league was congratulating them and the other half was already explaining why it absolutely would not happen again.':'Week '+week+' did not leave a clean scoring headliner.',
      champ?champ.team_name+' is the team I would least enjoy seeing across the bracket right now. They are '+rec(champ)+', '+(rank(champ)<999?ordinal(rank(champ))+' in the table, ':'')+'and the roster is giving off contender energy instead of one-week novelty. If that sounds premature, good. September is when a beat writer is supposed to develop a theory and spend the next three months either defending it or pretending the quote was taken out of context.':'The contender picture is still too thin to make a clean call.'
    ]},
    {reporter:reporterPublic(reporters[1]),heading:'Who I Believe — and Who I Don’t',paragraphs:[
      champ?'My early championship ticket is '+champ.team_name+'. I am planting the flag now, before the bracket makes it obvious. The record is '+rec(champ)+', but the part I trust more is that the scoring and recent form are not asking one miracle player to save them every Sunday. I would rather be wrong loudly in September than discover the obvious favorite in December and call it analysis.':'I am withholding the championship flag for one more week, which pains me more than it should.',
      fake&&champ&&fake.roster_id!==champ.roster_id?fake.team_name+' is my fraud alert. The standing looks healthier than the weekly performance underneath it, and I am predicting they are out before the conference championship unless that changes. Somebody has to be the team everyone respects until the first cold playoff Sunday exposes the drywall. I am volunteering them. Save the angry screenshots.':'I do not have a clean fraud candidate yet. Give the league another week to lie to us.'
    ]},
    {reporter:reporterPublic(reporters[2]),heading:'What Blew Up the Group Chat',paragraphs:[
      ...
      poy?poy.p.name+' is my Player of the Year pick today. '+one(poy.p.season_avg||poy.p.points)+' fantasy points per game is the headline, but the fun part is that '+poy.team.team_name+' now gets to build Sundays around somebody everyone else wishes they rostered. Awards are supposed to be argued about before they are obvious, so I am putting the name in ink now.':'I am not handing out a Player of the Year flag without a player sample worth arguing over.'
    ]},
    {reporter:reporterPublic(reporters[3]),heading:'The Next Thing That Could Get Weird',paragraphs:[
      up?'My upset special for next week is '+up.underdog.team_name+' over '+up.favorite.team_name+'. The standings say I am supposed to pick the other side; the recent scoring says I would rather be early than obedient. This is exactly the sort of prediction that looks inspired on Monday and gets quoted back at you by halftime, which is why it belongs here.':'The next-week slate does not give me a clean upset target yet.',
  ...
  if(divs.length)hot.push({kind:'division',reporter:reporterPublic(reporters[0]),title:'Planting the division flags now',take:divs.map(x=>x.division+': '+x.team.team_name).join(' • ')+'. These are picks, not current standings.'});
  if(poy)hot.push({kind:'player',reporter:reporterPublic(reporters[1]),title:'Player of the Year pick: '+poy.p.name,take:'Bartholomew Roycington III is taking '+poy.p.name+' for Player of the Year right now. The pick can age beautifully or become evidence.'});
  if(up)hot.push({kind:'upset',reporter:reporterPublic(reporters[2]),title:'Upset special: '+up.underdog.team_name+' over '+up.favorite.team_name,take:'Tilly Fleecer is calling the upset before kickoff: '+up.underdog.team_name+' beats '+up.favorite.team_name+'. If it misses, the back page will deny remembering this paragraph.'});

  return {schema_version:2,inquirer_version:18,season:Number(season),week:Number(week),week_classification:classification,headline:'Fleeced! League Notebook — '+(classification.label||('Week '+week)),byline:'By '+reporters.map(r=>r.name).join(', '),deck:'Four desks, one league, and enough confidence to be embarrassing by next Sunday.',sections,hot_takes:hot,bottom_five:rows.slice().sort((a,b)=>rank(b)-rank(a)).slice(0,5).map(t=>({roster_id:t.roster_id,team_name:t.team_name,rank:rank(t),record:t.league_context?.record||null,recent_avg_points:t.league_context?.recent_avg_points||null,streak:t.league_context?.streak||null})),value_history_meta:valueHistoryMeta,generated_from:'Sleeper matchups, standings, transactions, player metadata/stats, canonical Trade History, canonical team Value History, verified NFL schedule, and Sleeper injury designations'};
}
