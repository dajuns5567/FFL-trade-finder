...
export function buildHumanLeagueOverview({season,week,teams,players,transactions,canonicalTrades=[],weekClassification,reporters,valueHistoryMeta={}}){
  ...
  const sections=[
    ...
    {reporter:reporterPublic(reporters[1]),heading:'Who I Believe — and Who I Don’t',paragraphs:[
      ...
      fake&&champ&&fake.roster_id!==champ.roster_id?fake.team_name+' is my fraud alert. The standing looks healthier than the weekly performance underneath it, and I am predicting they are out before the conference championship unless that changes. That is a prediction, not a diagnosis, so please save the angry screenshots.':'I do not have a clean fraud candidate yet. Give the league another week to lie to us.'
    ]},
    {reporter:reporterPublic(reporters[2]),heading:'What Blew Up the Group Chat',paragraphs:[
      trade?'The transaction that has the newsroom talking: '+trade+' I do not need to declare a winner tonight; I only need to know which manager is going to pretend this was always the plan.':adds.length?adds.slice(0,3).map(x=>x.name+' landed with '+x.team).join(', ')+'. Nothing starts a group-chat argument faster than a waiver claim somebody else had queued.':'The trade wire stayed suspiciously calm, which means the league is either content or plotting.',
      poy?poy.p.name+' is my Player of the Year pick today. '+one(poy.p.season_avg||poy.p.points)+' fantasy points per game is the headline, but the fun part is that '+poy.team.team_name+' now gets to build Sundays around somebody everyone else wishes they rostered.':'I am not handing out a Player of the Year flag without a player sample worth arguing over.'
    ]},
    {reporter:reporterPublic(reporters[3]),heading:'The Next Thing That Could Get Weird',paragraphs:[
      up?'My upset special for next week is '+up.underdog.team_name+' over '+up.favorite.team_name+'. The standings say I am supposed to pick the other side; the recent scoring says I would rather be early than obedient.':'The next-week slate does not give me a clean upset target yet.',
      bottom?(bottom.team_name+' is sitting at the wrong end of the table at '+rec(bottom)+'. '+(pressure&&pressure.roster_id===bottom.roster_id?'The bye/injury board is making the escape route uglier, too.':'That is not destiny, but it is enough to start checking where the next win is supposed to come from.')):'There is no clean bottom-of-table story yet.'
    ]}
  ];
