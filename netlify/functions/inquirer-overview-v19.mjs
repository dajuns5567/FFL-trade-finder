...
export function buildHumanLeagueOverviewV19({season,week,teams,players,transactions,canonicalTrades=[],weekClassification,reporters,valueHistoryMeta={}}){
  ...
  const sections=[
    ...
    {reporter:reporterPublic(reporters[2]),heading:'What Blew Up the Group Chat',paragraphs:[
      trade?'TRADE RECEIPT: '+trade+' I do not need to declare a winner tonight; I only need to know which manager is going to pretend this was always the plan. Every trade has a honeymoon period, and in this league it usually lasts until the first player involved scores twice while sitting on somebody else’s bench.':adds.length?adds.slice(0,3).map(x=>x.name+' landed with '+x.team).join(', ')+'. Nothing starts a group-chat argument faster than a waiver claim somebody else had queued. By Tuesday morning, those claims will somehow become stories about who “knew” the breakout was coming.':'The trade wire stayed suspiciously calm, which means the league is either content or plotting.',
  ...
  const hot=[];
  if(champ)hot.push({kind:'championship',reporter:reporterPublic(reporters[0]),title:'Super Bowl pick: '+champ.team_name,take:'I’m calling '+champ.team_name+' to win the Fleeced! Super Bowl. No hedge, no “if healthy,” no emergency exit. If this ages badly, I expect the league chat to behave with its usual grace.'});
  if(fake)hot.push({kind:'fraud',reporter:reporterPublic(reporters[3]),title:'Fraud alert: '+fake.team_name,take:'I’m calling '+fake.team_name+' the early fraud team unless the weekly performance changes. A respectable record can wear a nice suit; eventually the scoring has to pay the bill.'});
  if(divs.length)hot.push({kind:'division',reporter:reporterPublic(reporters[0]),title:'Planting the division flags now',take:'I’m planting all eight flags early: '+divs.map(x=>x.team.team_name+' out of the '+x.division).join('; ')+'. Bookmark it now so the league has something specific to mock in December.'});
  if(poy)hot.push({kind:'player',reporter:reporterPublic(reporters[1]),title:'Player of the Year pick: '+poy.p.name,take:'I’m taking '+poy.p.name+' for Player of the Year right now. It is in ink, which is either conviction or a future exhibit in the league’s case against my judgment.'});
  if(up)hot.push({kind:'upset',reporter:reporterPublic(reporters[2]),title:'Upset special: '+up.underdog.team_name+' over '+up.favorite.team_name,take:'I’M CALLING THE PROJECTED UNDERDOG BEFORE KICKOFF: '+up.underdog.team_name+' ('+one(up.underdog_projected)+') over '+up.favorite.team_name+' ('+one(up.favorite_projected)+'). If it misses, I will deny remembering this paragraph with tremendous professionalism.',underdog_roster_id:String(up.underdog.roster_id),favorite_roster_id:String(up.favorite.roster_id),underdog_projected:up.underdog_projected,favorite_projected:up.favorite_projected,projection_gap:up.projection_gap});
