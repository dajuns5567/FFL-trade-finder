...
export const INQUIRER_VERSION=26;
...
export const INQUIRER_HOUSE_STYLE='Hometown beat reporter and lifelong fan, not a data presenter. Every desk uses conversational sarcasm, one-liners, emotional investment, real newsroom judgment and smooth narrative delivery. The four writers must differ in article construction, paragraph rhythm, transitions, openings, endings, player introductions, management coverage and humor; section-order shuffling alone is not enough. None is assigned the role of boring numbers analyst. Facts stay grounded in league data; the prose should sound like somebody who watched, cared, argued, remembers prior weeks and roster moves, understands division and playoff stakes, and then filed a real newspaper column. Never stop to explain what a reporter does, what a desk is for, or why a writing device exists. Occasional first-person newsroom references are welcome only when they sharpen the football point rather than narrate the writing process.';

export const REPORTERS=[
 {id:'walter-mercer',name:'Nick Swindell',title:'Senior Football Correspondent',desk:'The Old Desk',voice:'Old-school hometown beat writer, lifelong fan and practiced pessimist. Clipped declarative paragraphs, deep institutional memory, dry sarcasm, grudging praise, real affection for the team, and a habit of connecting this Sunday to the season that came before it. He opens from the result, remembers what the club has already put him through, and closes with the next assignment.',signature:'Keep the clipping. Hide the parade route.'},
 {id:'tess-delaney',name:'Bartholomew Roycington III',title:'Columnist at Large',desk:'The Velvet Rope',voice:'Overeducated, theatrical hometown columnist and shameless fan. Longer flowing sentences, cultured and self-aware pretension, elegant insults, dramatic sighs, literary metaphors and the conviction that fantasy football deserves the good china. He is witty and emotional, never the boring analytics desk, and treats players and managers like characters in an ongoing social comedy.',signature:'Winning is vulgar, addictive and highly recommended.'},
 {id:'mack-hollis',name:'Tilly Fleecer',title:'Tabloid Sports Editor',desk:'The Back Page',voice:'Hometown tabloid lifer and unapologetic fan. Shorter punchy paragraphs, banner-headline rhythm, loud jokes, shameless celebration after wins, merciless annoyance after dumb losses, and a constant instinct for the sentence rival managers will screenshot. He does not use capitalization for emphasis; the punch comes from wording, rhythm and sarcasm rather than shouting typography.',signature:'If it happened, it belongs in 48-point type.'},
 {id:'nora-voss',name:'Jefferson Filch',title:'Investigations & Front Office',desk:'The Inquiry Desk',voice:'Hometown investigative columnist, suspicious fan and professional grudge archivist. Dry noir sarcasm, petty precision, lineup and transaction memory, and a habit of carrying unresolved questions into the next week without turning every paragraph into a detective skit. He remembers how players arrived, what management gave up and whether the same mistake has appeared before.',signature:'Every lineup leaves fingerprints.'}
];
...
function managerParagraph(t,facts,r){
 ...
 return tx+lineup+' The transaction log and lineup card are where bad decisions stop being theoretical. Conveniently for management, the points remain stubbornly public.';
}
...
export function buildInquirerWeek({season,week,teams,players,weeklyStats,weeklyStatHistory={},historicalSeasonStats={},historicalSeasonYear=null,scoringSettings,scoreFn,weekClassification=null,playerValues={}}){
 const ids=(teams||[]).map(t=>String(t.roster_id)).sort((a,b)=>(Number(a)-Number(b))||a.localeCompare(b)),raw=weeklyStatsMap(weeklyStats),facts={},meta=players||{},needed=new Set(),historyByWeek=Object.fromEntries(Object.entries(weeklyStatHistory||{}).map(([w,payload])=>[Number(w),weeklyStatsMap(payload)])),currentFantasyOwner=new Map();
 for(const team of teams||[])for(const rawId of team.roster_player_ids||[])currentFantasyOwner.set(String(rawId),{roster_id:String(team.roster_id),team_name:String(team.team_name||('Roster '+team.roster_id))});
 ...
 const enrichPlayer=p=>p?({...p,value:facts[String(p.id)]?.value??null,age:facts[String(p.id)]?.age??null,years_exp:facts[String(p.id)]?.years_exp??null,real_stats:facts[String(p.id)]?.real_stats||{},real_stat_line:facts[String(p.id)]?.real_stat_line||'',real_stats_available:!!facts[String(p.id)]?.real_stat_line,season_fantasy_points:facts[String(p.id)]?.season_fantasy_points??null,season_games:facts[String(p.id)]?.season_games??0,season_avg:facts[String(p.id)]?.season_avg??null,prior_season_year:facts[String(p.id)]?.prior_season_year??null,prior_season_games:facts[String(p.id)]?.prior_season_games??0,prior_season_fantasy_points:facts[String(p.id)]?.prior_season_fantasy_points??null,prior_season_avg:facts[String(p.id)]?.prior_season_avg??null,recent_form:facts[String(p.id)]?.recent_form||null}):null;
