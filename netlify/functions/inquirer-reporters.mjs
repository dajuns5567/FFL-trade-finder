...
function hotTakeRows(teams,reporters){
 ...
 const streakTeam=rows.slice().sort((a,b)=>Number(b?.league_context?.streak?.length||0)-Number(a?.league_context?.streak?.length||0))[0];
 if(streakTeam)takes.push({reporter:reporterPublic(reporters[0]),title:'Nick Swindell: This is becoming a season, not a week',take:streakTeam.team_name+' is sitting on a '+Number(streakTeam?.league_context?.streak?.length||0)+'-game '+(streakTeam?.league_context?.streak?.type==='W'?'winning':'losing')+' streak. If the scoring pace and lineup process keep matching the results, the rest of the league has to stop treating this as temporary noise.'});
 ...
 if(managerMiss)takes.push({reporter:reporterPublic(reporters[2]),title:'Tilly Fleecer: Manager malpractice watch, presented with love',take:managerMiss.t.team_name+' had a '+one(Math.max(0,managerMiss.swing))+'-point gap between its best bench scorer and weakest starter. That does not make the decision wrong by hindsight alone, but if the same player-selection pattern repeats, I will personally upgrade this from “unfortunate” to “a bit.”'});
 ...
 if(worst)takes.push({reporter:reporterPublic(reporters[3]),title:'Jefferson Filch: The bottom of the table has consequences',take:worst.team_name+' currently sits #'+worst.league_context.standings_rank+' with a '+recordText(worst.league_context)+' record. If the losses continue, this is not only a bad season; it becomes a draft-position story. '+(pressure&&pressure.roster_id===worst.roster_id?'The next-week bye/injury pressure does not improve the defense.':'And yes, this desk is already documenting who has the schedule and roster depth to escape the basement.')});
 return takes;
}
