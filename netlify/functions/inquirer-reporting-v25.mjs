...
function nickExpansion(t,kind){
  ...
  if(kind==='players'&&top)return [(rows[1]?rows[1].name+' matters here because good teams stop asking the same star to rescue them every week. ':'')+'The names below the headline decide whether this lineup travels when the schedule gets less friendly.','What I want to see next is whether the middle of the lineup can make an ordinary Sunday feel safe. Stars win weeks; depth is what keeps every week from becoming an emergency.'];
...
function bartholomewExpansion(t,kind){
  ...
  if(kind==='players'&&top)return [top.name+' supplied the star turn; the season becomes genuinely interesting when the rest of the cast makes that level of performance feel less exceptional and more like the house style.','The supporting cast now has a choice: become recurring characters or remain beautifully dressed extras. One is how a roster grows dangerous; the other is how a columnist develops a recurring complaint.'];
...
function tillyExpansion(t,kind){
  ...
  if(kind==='players'&&top)return [top.name+' gets the banner. '+(second?second.name+' gets the next column. ':'The supporting cast gets a challenge.')+'If this lineup wants to become a weekly problem for the league, the names below them have to keep showing up too.','The back page can survive one superstar carrying the edition. A contender cannot. Somebody else needs to keep earning ink before the headline act gets tired of doing all the work.'];
...
function filchExpansion(t,kind){
  ...
  if(kind==='players'&&top)return ['The production around '+top.name+' matters because roster dependence leaves fingerprints over time. If the same supporting names keep appearing, that is depth; if they vanish, this week becomes an outlier with excellent publicity.','The next file will compare the same names again. Repeated support turns a strong performance into roster structure; disappearing support turns it into evidence that one player is doing too much of the work.'];
...
function availabilityStories(teams){
  ...
  const second=`${x.t.team_name} does not need to turn ${x.p.name}’s designation into melodrama, but it does need a Plan B before ${op} arrives. Availability is the least glamorous part of roster construction right up until a contender discovers that its expensive depth was decorative.`;
  return[first,second];
}
...
export function expandWeeklyRecapV25(o,teams,week){
  ...
  const upMove=movement(teams,1),downMove=movement(teams,-1),trend=playerTrend(teams),availability=availabilityStories(teams),moves=teams.map(t=>({t,text:managementStory(t)})).filter(x=>x.text).sort((a,b)=>Number(b.t.current_week_trade_count||0)-Number(a.t.current_week_trade_count||0)||(b.t.transactions?.length||0)-(a.t.transactions?.length||0)).slice(0,2),next=nextGame(teams);
  const velvet=[
  ...
  const sections=[
    ...
    {reporter:reporter(2),heading:'The Back Page Has Receipts',paragraphs:moves.length?moves.map(x=>x.text):['No completed move cleared the bar for a league-wide transaction story this week. The wire can be busy without being important.']},
