...
function playerStatInsightV33(t,p,r){
  ...
  if(pos==="QB"){
    ...
    football=keyedChoice(key,[
      ...
      takeaway?name+" gave the matchup the kind of defensive jolt that changes a sideline instantly. "+q.opp+" went from building a drive to dealing with the consequences of losing the football.":keyedChoice(key+":idp-disruption",[name+" made "+q.opp+" work harder for its offense. "+q.opp+" spent too many snaps accounting for where "+name+" was coming from.",name+" kept showing up in the part of the game "+q.opp+" wanted to make comfortable. By the end, the defense had turned his presence into one more thing the offense had to solve.",q.opp+" never got a completely clean afternoon because "+name+" kept inserting himself into possessions that were supposed to be ordinary. "+["The offense kept paying an annoyance tax every time it tried to settle in.","That made routine possessions feel less routine by the fourth quarter.","The useful consequence was visible in how often the offense had to account for him before the snap.","It left the offense spending attention on a defender it would rather have ignored.","The disruption mattered because it kept clean possessions from staying clean.","By the end, the offense was planning around a defender it had hoped to treat as background noise.","That kind of nuisance changes play-calling even when the box score does not advertise every effect.","The matchup kept carrying his fingerprints long after the individual play ended."][Math.abs(Number(t.roster_id)||0)%8],name+" did not need a turnover to make himself felt. "+q.opp+" kept having to finish drives with "+name+" somewhere in the problem, which is exactly the kind of defensive nuisance "+q.team+" needed."])
    ]);
  }
  const close=[
...
function playerStoryV29(t,r,f=articleFrameV29(t,r)){
  ...
  if(other.length&&threeHighScorersV33(f)){
    ...
    const close=[
      ...
      won?keyedChoice(`${t.roster_id}:tilly-support-win`,[`${team} had three actual high-end scorers, so ${top.name} did not have to carry the whole result.`,`The ${team} win had three players over the high-scorer threshold. That is enough real production to call the support meaningful.`,`${top.name} had serious help from the other two high scorers, which is the rare week where the group-performance point is actually earned.`]):`Good ${team} performances do not erase the loss; they make the quiet lineup spots easier to identify.`,
...
export function expandWeeklyRecapV25(o,teams,week){
  ...
  const bartholomewRecapRead=upset?upset.loser.team_name+" arrived with the nicer forecast and left "+upset.winner.team_name+" holding the better evening. That is the sort of result Bartholomew adores because entitlement looks dreadful under stadium lighting. The underdog gets swagger; the favorite gets a week of explaining why the expensive roster still got its pocket picked.":close?close.winner.team_name+" and "+close.loser.team_name+" gave the league the least elegant kind of entertainment: a game close enough that every bad lineup choice suddenly looks personally insulting. One side gets relief. The other gets hindsight with sharp elbows.":"Week 1 already gave Bartholomew enough material to distinguish the teams that controlled a room from the ones that merely dressed for it.";
