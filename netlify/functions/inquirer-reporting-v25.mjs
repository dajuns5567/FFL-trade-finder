...
function teamPlayerExtraV33(t,r,f,slot){
  ...
  const banks=[
    [
      ...
      [top.name+' is the player '+team+' most needs to carry forward. The next opponent gets a different game environment, and the question is whether the same role still creates the same pressure.','For '+team+', '+top.name+' now has to show the workload survives when the opponent and game script change. That is what would turn one strong Sunday into something dependable.','The next question for '+top.name+' is whether the same role shows up again. A second comparable workload would say much more than another celebration of this week’s total.','For '+top.name+', the next test is whether this '+roleLabel+' role still changes the matchup after '+shapePhrase+'; everybody else can earn their own share of the attention.']
    ],
    [
