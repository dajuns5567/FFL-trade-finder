...
function nextOpponentLeagueContextV37(t,r){
  ...
  ][Math.abs(Number(t.roster_id)||0)%16]:`${opp} comes off a win at ${recText}; the form is positive even if it has not become a streak yet.`;
  ...
  ][Math.abs(Number(t.roster_id)||0)%16]:`${opp} comes off a loss at ${recText}; the next game is already carrying response-game weight.`;
...
function nextOpponentLeadV29(t,r,f=articleFrameV29(t,r)){
  const o=t.next_opponent_roster,opp=String(t.next_opponent_name||o?.team_name||'the next opponent'),rows=(o?.starters||o?.players||[]).filter(p=>valid(p?.points)).slice().sort((a,b)=>Number(b.points)-Number(a.points)),
    ...
    recText=rec?`${Number(rec.wins)||0}-${Number(rec.losses)||0}`:null,starText=star?`${star.name} just produced ${one(star.points)} fantasy points${clause?`; ${star.name} ${clause}`:''}.`:'',depthText=nextOpponentDepthV29(o,star,r),leagueText=nextOpponentLeagueContextV37(t,r);
  if(voice(r)===0){
