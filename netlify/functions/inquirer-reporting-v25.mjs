...
function nextOpponentLeagueContextV37(t,r){
  ...
  else if(streakType==='W')form=games===1?keyedChoice(key+':one-win',[`${opp} opened ${recText}; one win is not a heater yet, but it means the opponent arrives believing its Week 1 formula worked.`,`${opp} is ${recText} after an opening win; ${t.team_name} gets the first chance to make that confidence look temporary.`,`${opp} brings a ${recText} record and a fresh win into this matchup; it will expect the good feeling to travel.`,`${opp} opened ${recText}; this is a team coming in pleased with itself, which makes an early punch more valuable.`,`${opp} is ${recText}; the opener went its way, so ${t.team_name} should prepare for a roster expecting continuity rather than repair.`,`${opp} arrives ${recText} after winning its first game; stopping the momentum now is easier than chasing it later.`,`${opp} starts this matchup at ${recText}; it has not earned the word hot, but it has earned the confidence that comes with winning.`,`${opp} won its opener and sits ${recText}; ${t.team_name} gets a chance to turn that good start into a one-week memory.`]):`${opp} comes off a win at ${recText}; the form is positive even if it has not become a streak yet.`;
  else if(streakType==='L')form=games===1?keyedChoice(key+':one-loss',[`${opp} opened ${recText}; it is not cold after one game, but it is already carrying a response game into this matchup.`,`${opp} is ${recText} after an opening loss; ${t.team_name} should make the search for its first clean answer last another week.`,`${opp} arrives ${recText}; one loss does not define it, but it does make the next result feel more urgent.`,`${opp} opened ${recText}; expect a team trying to correct itself, then make that correction as difficult as possible.`,`${opp} is ${recText}; the first Sunday went badly enough that this matchup already comes with response-game energy.`,`${opp} brings an ${recText} start into this one; ${t.team_name} should treat the opponent’s urgency as something to exploit, not fear.`,`${opp} lost its opener and sits ${recText}; the next win would feel like relief, which is exactly why ${t.team_name} should deny it.`,`${opp} starts this matchup ${recText}; it is looking for a reset, and ${t.team_name} should make sure the reset button stays out of reach.`]):`${opp} comes off a loss at ${recText}; the next game is already carrying response-game weight.`;
