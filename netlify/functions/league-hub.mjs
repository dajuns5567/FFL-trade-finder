...
async function tradeReports(req){const s=store();if(req.method==='POST'){const body=await req.json().catch(()=>null),r=body?.report;if(!r?.trade_id)return json({error:'invalid trade report'},400);const key='inquirer/trade-reports/'+String(r.trade_id).replace(/[^a-zA-Z0-9_-]/g,'_')+'.json',existing=await s.get(key,{type:'json'}).catch(()=>null);if(!existing||Number(existing.report_version||0)<Number(r.report_version||0)){await s.setJSON(key,{...r,type:'trade',captured_at:existing?.captured_at||new Date().toISOString(),updated_at:new Date().toISOString()})}const idx=await s.get('inquirer/trade-reports/index.json',{type:'json'}).catch(()=>[]),list=Array.isArray(idx)?idx:[];if(!list.some(x=>String(x.trade_id)===String(r.trade_id))){list.push({type:'trade',trade_id:String(r.trade_id),created:Number(r.created)||Date.now(),key});list.sort((a,b)=>Number(b.created)-Number(a.created));await s.setJSON('inquirer/trade-reports/index.json',list)}return json({ok:true})}const idx=await s.get('inquirer/trade-reports/index.json',{type:'json'}).catch(()=>[]);return json({reports:Array.isArray(idx)?idx:[]})}
async function tradeReportStored(id){const s=store(),safe=String(id||'').replace(/[^a-zA-Z0-9_-]/g,'_'),v=await s.get('inquirer/trade-reports/'+safe+'.json',{type:'json'}).catch(()=>null);return json(v||{error:'trade report not found'},v?200:404)}
