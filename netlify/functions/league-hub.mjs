...
async function managerHistory(){
 ...
 for(const lg of chain){const lid=String(lg.league_id),season=Number(lg.season),[rosters,users,bracket]=await Promise.all([fetchJson(`${API}/league/${lid}/rosters`).catch(()=>[]),fetchJson(`${API}/league/${lid}/users`).catch(()=>[]),fetchJson(`${API}/league/${lid}/winners_bracket`).catch(()=>[])]);
  const ub=new Map(users.map(u=>[String(u.user_id),u])),rb=new Map(rosters.map(r=>[String(r.roster_id),r])),ownerByRoster={};for(const r of rosters){const uid=String(r.owner_id||'');if(!uid)continue;ownerByRoster[String(r.roster_id)]=uid;const u=ub.get(uid)||{};ensure(uid,u);assignments.push({season,roster_id:String(r.roster_id),user_id:uid,sleeper_id:career[uid].sleeper_id});if(String(lg.league_id)===String(LEAGUE))current.push({roster_id:String(r.roster_id),user_id:uid,sleeper_id:career[uid].sleeper_id})}
  const regularMax=season<currentSeason?14:Math.min(14,Math.max(0,currentWeek-1)),seasonWins={},seasonPoints={};
  for(let w=1;w<=regularMax;w++){const [ms,proj]=await Promise.all([fetchJson(`${API}/league/${lid}/matchups/${w}`).catch(()=>[]),projections(season,w,lg?.scoring_settings||{}).catch(()=>({}))]),groups={};for(const m of ms){const k=String(m.matchup_id??'');if(k)(groups[k]||(groups[k]=[])).push(m)}for(const pair of Object.values(groups)){if(pair.length!==2)continue;const [a,b]=pair;for(const [m,o] of [[a,b],[b,a]]){const uid=ownerByRoster[String(m.roster_id)];if(!uid)continue;const pts=Number(m.points)||0,opt=Number(o.points)||0,won=pts>opt;career[uid][won?'wins':'losses']++;seasonPoints[uid]=(seasonPoints[uid]||0)+pts;if(won)seasonWins[uid]=(seasonWins[uid]||0)+1;const rd=rb.get(String(m.roster_id))?.settings?.division,od=rb.get(String(o.roster_id))?.settings?.division;if(won&&rd!=null&&od!=null&&String(rd)===String(od))career[uid].division_wins++;const starters=(m.starters||[]).filter(x=>x&&x!=='0'),projected=starters.reduce((n,p)=>n+(Number(proj[p])||0),0),projection_coverage=starters.filter(p=>Number.isFinite(Number(proj[p]))).length;games.push({season,week:w,user_id:uid,roster_id:String(m.roster_id),points:pts,opponent_points:opt,won,playoff:false,projected:Number(projected.toFixed(2)),projection_delta:projection_coverage?Number((pts-projected).toFixed(2)):null,projection_coverage,starter_count:starters.length})}}}
