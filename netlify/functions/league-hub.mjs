...
async function managerHistory(){
 ...
 for(const lg of chain){
  ...
  for(const r of rosters){const uid=String(r.owner_id||'');if(!uid)continue;ownerByRoster[String(r.roster_id)]=uid;const u=ub.get(uid)||{};if(!career[uid])career[uid]={user_id:uid,sleeper_id:String(u.display_name||u.username||uid),wins:0,championships:0,division_wins:0,division_championships:0,regular_season_titles:0,seasons:[]};assignments.push({season,roster_id:String(r.roster_id),user_id:uid,sleeper_id:career[uid].sleeper_id});if(String(lg.league_id)===String(LEAGUE))current.push({roster_id:String(r.roster_id),user_id:uid,sleeper_id:career[uid].sleeper_id})}
  const regularMax=season<currentSeason?14:Math.min(14,Math.max(0,currentWeek-1)),seasonWins={},seasonPoints={};
  const countWeek=async(w,winnersOnly=false)=>{const ms=await fetchJson(`${API}/league/${lid}/matchups/${w}`).catch(()=>[]),groups={};for(const m of ms){const k=String(m.matchup_id??'');if(!k)continue;(groups[k]||(groups[k]=[])).push(m)}for(const pair of Object.values(groups)){if(pair.length!==2)continue;const [a,b]=pair,ap=Number(a.points)||0,bp=Number(b.points)||0,aw=ap>bp,bw=bp>ap;for(const [m,won,o] of [[a,aw,b],[b,bw,a]]){const uid=ownerByRoster[String(m.roster_id)];if(!uid)continue;if(w<=14){seasonPoints[uid]=(seasonPoints[uid]||0)+(Number(m.points)||0);if(won){seasonWins[uid]=(seasonWins[uid]||0)+1;career[uid].wins++;const rd=rb.get(String(m.roster_id))?.settings?.division,od=rb.get(String(o.roster_id))?.settings?.division;if(rd!=null&&od!=null&&String(rd)===String(od))career[uid].division_wins++}}else if(won&&winnersOnly)career[uid].wins++}}};
  for(let w=1;w<=regularMax;w++)await countWeek(w);
  const seasonComplete=season<currentSeason;
  if(seasonComplete){
    // Post-Week-14 wins count only for teams that Sleeper placed in the winners bracket.
    const winnerRosters=new Set((bracket||[]).flatMap(x=>[x?.r,x?.w,x?.l]).filter(x=>x!=null).map(String));
    for(let w=15;w<=18;w++){const ms=await fetchJson(`${API}/league/${lid}/matchups/${w}`).catch(()=>[]),groups={};for(const m of ms){const k=String(m.matchup_id??'');if(!k)continue;(groups[k]||(groups[k]=[])).push(m)}for(const pair of Object.values(groups)){if(pair.length!==2)continue;const [a,b]=pair,ap=Number(a.points)||0,bp=Number(b.points)||0;for(const [m,won] of [[a,ap>bp],[b,bp>ap]]){const rid=String(m.roster_id),uid=ownerByRoster[rid];if(uid&&won&&winnerRosters.has(rid))career[uid].wins++}}}
  ...
  // All regular-season honors freeze at Week 14, never using playoff results.
  if(regularMax>=14){let top=null;const divTop={};for(const r of rosters){const rid=String(r.roster_id),uid=ownerByRoster[rid];if(!uid)continue;const wins=seasonWins[uid]||0,pts=seasonPoints[uid]||0,score=wins*1e9+pts;if(!top||score>top.score)top={uid,score};const d=r.settings?.division;if(d!=null&&(!divTop[d]||score>divTop[d].score))divTop[d]={uid,score}}if(top)career[top.uid].regular_season_titles++;for(const x of Object.values(divTop))career[x.uid].division_championships++}
 }
