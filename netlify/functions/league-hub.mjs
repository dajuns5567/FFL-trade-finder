...
import {INQUIRER_VERSION,publicReporters,buildInquirerWeek} from './inquirer-reporters.mjs';
...
async function syncReporterArchives(s,result,broadcastKey){
 ...
 for(const reporter of reporters){
  ...
  for(const team of teams.filter(t=>t?.inquirer_article?.reporter?.id===reporter.id)){
   ...
   if(!stored?.headline||migrate)await s.setJSON(articleKey,{...article,team_name:String(team.team_name||''),manager_name:String(team.manager_name||''),captured_at:String(result.generated_at||new Date().toISOString()),migration_reason:migrate&&stored?.headline?'explicit V10 league/player-context upgrade':null});
...
async function weeklyReport(){
 ...
 const probeWeeks=[...new Set([currentWeek,Math.max(1,currentWeek-1)])];
 let week=null;
 ...
 await s.setJSON(key,result);await syncReporterArchives(s,result,key);
