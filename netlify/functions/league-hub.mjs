...
async function managerHistory(){
 ...
 for(const lg of chain){const lid=String(lg.league_id),season=Number(lg.season),regularMax=season<currentSeason?13:Math.min(13,Math.max(0,currentWeek-1));
  ...
  if(season<currentSeason){const playoffWeeks=await Promise.all([14,15,16,17].map(w=>fetchJson(`${API}/league/${lid}/matchups/${w}`).catch(()=>[]))),bracketRosters=new Set((bracket||[]).flatMap(x=>[x?.r,x?.w,x?.l]).filter(x=>x!=null).map(String));for(let i=0;i<playoffWeeks.length;i++){const w=14+i,groups={};for(const m of playoffWeeks[i]){const k=String(m.matchup_id??'');if(k)(groups[k]||(groups[k]=[])).push(m)}for(const pair of Object.values(groups)){if(pair.length!==2)continue;const [a,b]=pair;if(!bracketRosters.has(String(a.roster_id))||!bracketRosters.has(String(b.roster_id)))continue;const ap=Number(a.points)||0,bp=Number(b.points)||0;if(ap===bp)continue;const win=ap>bp?a:b,lose=ap>bp?b:a;for(const [m,o,won] of [[win,lose,true],[lose,win,false]]){const uid=ownerByRoster[String(m.roster_id)];if(!uid)continue;career[uid][won?'wins':'losses']++;if(won)career[uid].playoff_wins++;games.push({season,week:w,user_id:uid,roster_id:String(m.roster_id),points:Number(m.points)||0,opponent_points:Number(o.points)||0,won,playoff:true})}}}
