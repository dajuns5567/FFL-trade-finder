...
function isIdp(row){
  return String(row?.pos||'').toUpperCase()==='IDP';
}
...
export default async (req)=>{
  if(process.env.CONTEXT!=='deploy-preview')return json({error:'maintenance endpoint is deploy-preview only'},403);
  ...
  if(report.offenseRowsBefore!==report.offenseRowsAfter){
    return json({error:'offense preservation assertion failed',report},500);
  }

  const latest=await store.get('latest.json',{type:'json'}).catch(()=>null);
  if(latest?.key&&isPre(latest.t)){
    const snap=changed.get(String(latest.key))||await store.get(String(latest.key),{type:'json'}).catch(()=>null);
  ...
  if(remainingIdp!==0)return json({error:'pre-cutoff IDP rows remain',report},500);
  if(verifiedOffense!==report.offenseRowsBefore)return json({error:'offense row verification mismatch',report},500);
