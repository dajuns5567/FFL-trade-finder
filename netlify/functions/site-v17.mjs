...
export default async()=>{
  try{
    const [r,coreR,cssR,v106JsR,v106CssR]=await Promise.all([
    ...
    const directSection1='<script>'+section1Core+'\nwindow.tradeSection1V100?.install?.();\n'+section1V106Js+'\nwindow.__section1Release="v106";window.section1V106?.install?.();</script>';
    const html=sourceUi.replace("</body>",scripts+directSection1+"</body>");
    return new Response(html,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-fll-release":"section1-v106"}});
