...
function localText(name){
  const candidates=[join(process.cwd(),name),new URL(`../../${name}`,import.meta.url)];
  let last;
  for(const p of candidates){try{return readFileSync(p,'utf8')}catch(e){last=e}}
  throw last||new Error(`Unable to read packaged ${name}`);
}

let cachedHtml=null;

function buildHtml(){
  const raw=localText('index.html');
  const section1Core=localText('trade-section1-core-v100.js').replace(/<\/script/gi,"<\\/script");
  const section1Css=localText('trade-section1-v100.css');
  const section1V106Css=localText('trade-section1-v106.css');
  ...
  return sourceUi.replace("</body>",scripts+releaseSeed+core+v116+"</body>");
}

export default async()=>{
  try{
    if(cachedHtml===null)cachedHtml=buildHtml();
    return new Response(cachedHtml,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-fll-release":"section1-v116"}});
  }catch(e){
    cachedHtml=null;
    return new Response(`Site loader failed: ${String(e?.message||e)}`,{status:500,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}})
  }
};