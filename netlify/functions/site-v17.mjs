...
const SECTION1_V106_JS_URL=RAW_BASE+"trade-section1-v106.js";
const SECTION1_V106_CSS_URL=RAW_BASE+"trade-section1-v106.css";
const SECTION1_V110_JS_URL=RAW_BASE+"trade-section1-v110.js";
const SECTION1_V111_JS_URL=RAW_BASE+"trade-section1-v111.js";
export default async()=>{
  try{
    const [r,coreR,cssR,v106JsR,v106CssR,v110JsR,v111JsR]=await Promise.all([
      ...
      fetch(SECTION1_V106_JS_URL,{headers:{accept:"text/javascript"},cache:"no-store"}),
      fetch(SECTION1_V106_CSS_URL,{headers:{accept:"text/css"},cache:"no-store"}),
      fetch(SECTION1_V110_JS_URL,{headers:{accept:"text/javascript"},cache:"no-store"}),
      fetch(SECTION1_V111_JS_URL,{headers:{accept:"text/javascript"},cache:"no-store"})
    ]);
    ...
    if(!v106JsR.ok)throw new Error(`Section 1 V106 JS ${v106JsR.status}`);
    if(!v106CssR.ok)throw new Error(`Section 1 V106 CSS ${v106CssR.status}`);
    if(!v110JsR.ok)throw new Error(`Section 1 V110 JS ${v110JsR.status}`);
    if(!v111JsR.ok)throw new Error(`Section 1 V111 JS ${v111JsR.status}`);
    const raw=await r.text();
    const section1Core=(await coreR.text()).replace(/<\/script/gi,"<\\/script");
    const section1Css=await cssR.text();
    const section1V106Js=(await v106JsR.text()).replace(/<\/script/gi,"<\\/script");
    const section1V106Css=await v106CssR.text();
    const section1V110Js=(await v110JsR.text()).replace(/<\/script/gi,"<\\/script");
    const section1V111Js=(await v111JsR.text()).replace(/<\/script/gi,"<\\/script");
    ...
    const directSection1='<script>'+section1Core+'\nwindow.tradeSection1V100?.install?.();\n'+section1V106Js+'\nwindow.section1V106?.install?.();\n'+section1V110Js+'\n'+section1V111Js+'\nwindow.section1V111?.install?.();</script>';
    const v113='<script src="/trade-section1-v113.js?v=113"></script><script>window.section1V113?.install?.();</script>';
    const v114='<script src="/trade-section1-v114.js?v=114"></script><script>window.section1V114?.install?.();</script>';
    const v115='<script src="/trade-section1-v115.js?v=115"></script><script>window.__section1Release="v115";window.section1V115?.install?.();</script>';
    const html=sourceUi.replace("</body>",scripts+directSection1+v113+v114+v115+"</body>");
    return new Response(html,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-fll-release":"section1-v115"}});
