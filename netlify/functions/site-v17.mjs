...
export default async()=>{
  try{
    ...
    const directSection1='<script>'+section1Core+'\nwindow.tradeSection1V100?.install?.();\n'+section1V106Js+'\nwindow.section1V106?.install?.();\n'+section1V110Js+'\n'+section1V111Js+'\nwindow.__section1Release="v111";window.section1V111?.install?.();</script>';
    const html=sourceUi.replace("</body>",scripts+directSection1+"</body>");
    return new Response(html,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-fll-release":"section1-v111"}});
