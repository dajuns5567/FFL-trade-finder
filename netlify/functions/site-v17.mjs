...
export default async()=>{
  try{
    ...
    const sourceUi=deferred
      ...
      .replace('<div><h1>FLL 32-Team Dynasty Trade Finder</h1><p>','<div><h1 class="fleecedFlat113" aria-label="Fleeced!"><span class="fleecedWord113">Fleeced!</span><span class="fleecedArrow113" aria-hidden="true"></span></h1><p>');
    ...
    const directSection1='<script>window.__section1Release="v113";'+section1Core+'\nwindow.tradeSection1V100?.install?.();\n'+section1V106Js+'\nwindow.section1V106?.install?.();\n'+section1V110Js+'\n'+section1V111Js+'\nwindow.section1V111?.install?.();</script>';
    const v113='<script src="/trade-section1-v113.js?v=113"></script><script>window.section1V113?.install?.();</script>';
    const html=sourceUi.replace("</body>",scripts+directSection1+v113+"</body>");
    return new Response(html,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-fll-release":"section1-v114-logo-race-fix"}});
