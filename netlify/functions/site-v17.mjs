...
export default async()=>{
  try{
    const [r,coreR,cssR,v106JsR,v106CssR,v110JsR]=await Promise.all([
    ...
    const directSection1='<script>'+section1Core+'\nwindow.tradeSection1V100?.install?.();\n'+section1V106Js+'\nwindow.section1V106?.install?.();\n'+section1V110Js+'\nwindow.__section1Release="v110";</script>';
    const html=sourceUi.replace("</body>",scripts+directSection1+"</body>");
    return new Response(html,{status:200,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store","x-fll-release":"section1-v110"}});
