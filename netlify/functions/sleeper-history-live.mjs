import {PPR_WEIGHTS,aggregateWeeks,valuationEligibleCurrentSeasonWeeks} from './ppr-scoring.mjs';
...
async function weekFinality(season,week){
  ...
  const events=Array.isArray(payload?.events)?payload.events:[];let finalGames=0;
  for(const event of events){const status=event?.competitions?.[0]?.status?.type;if(status?.completed===true||String(status?.name||'').toUpperCase()==='STATUS_FINAL')finalGames++}
  return{scheduledGames:events.length,finalGames,complete:events.length>0&&finalGames===events.length};
}
