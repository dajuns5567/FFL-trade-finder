...
const headers={accept:'application/json','user-agent':'FFL-TradeFinder-SleeperHistory/1.1'};
...
function weightPlan(currentSeason,completedWeek){
  ...
  if(w>=18)return{mode:'postseason-offseason',completedWeek:w,weights:{currentYear:0,previousYear:.60,twoYearsAgo:.30,threeYearsAgo:.10},yearWeights:{[currentSeason]:.60,[currentSeason-1]:.30,[currentSeason-2]:.10}};
...
export default async function handler(req){
  try{
    ...
    for(let i=0;i<4&&id;i++){const league=await getJson(`${API}/league/${id}`),season=Number(league?.season);if(!season)break;chain.push({leagueId:id,season,scoringSettings:league?.scoring_settings||{},settings:league?.settings||{},previousLeagueId:league?.previous_league_id?String(league.previous_league_id):null});id=league?.previous_league_id?String(league.previous_league_id):null}
    ...
    const plan=weightPlan(currentSeason,completedWeek);
    return new Response(JSON.stringify({ok:true,generatedAt:new Date().toISOString(),currentLeagueId:start,currentSeason,completedWeek,weightPlan:plan,qualifyingHistoricalSeasonMinimumGames:8,chain,aggregatedBySeason,rosterMutation:false,notes:['This endpoint does not fetch or modify live rosters.','Current-year stats may contribute from Week 1 using the dynamic weekly weight.','Historical completed-season samples remain subject to the 8-game minimum.']}),{status:200,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}});
