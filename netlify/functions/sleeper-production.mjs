...
export default async function handler(req){
  ...
  const headers={accept:'application/json','user-agent':'FFL-TradeFinder/production-anchor-1.2'};
  const out={},errors=[];
  let leagueStatus='';
  ...
  const plan=weightPlan(season,completedWeek,leagueStatus);
  return new Response(JSON.stringify({ok:true,season,currentSeason:season,years,stats:out,aggregatedBySeason:out,errors,completedWeek,weightPlan:plan,pprScoringWeights:PPR_WEIGHTS,pprReconstruction:true,fallback:true}),{status:200,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store'}});
}
