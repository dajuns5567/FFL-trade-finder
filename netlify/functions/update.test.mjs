...
import { buildConsensusPayload } from "./update.mjs";

test("adapts valid rankings and retains diagnostics for every source",()=>{
  const results=Array.from({length:8},(_,index)=>({
    ...
    rankings:index<2?[
      {player:"  Ja'Marr Chase  ",rank:1},
      {player:"Justin Jefferson",rank:2}
    ]:[],
  ...
  const payload=buildConsensusPayload({results},"2026-08-13T01:00:00.000Z");

  assert.deepEqual(Object.keys(payload.sources),["Source 1","Source 2"]);
  assert.deepEqual(payload.sources["Source 1"].data,{
    "ja'marr chase":1,
    "justin jefferson":2
  });
