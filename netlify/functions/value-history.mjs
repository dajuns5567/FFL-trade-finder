...
const isKnownBadHistoryTime=t=>isV380BadTime(t)||isV381BadTime(t)||isV391BadTime(t)||isV486BadTime(t)||isV487BadTime(t)||isV490BadTime(t)||isV492BadTime(t)||isV494BadTime(t);
...
async function scrubV380PartialWeekSnapshot(s){
  const marker=await safeGet(s,V380_CLEANUP_KEY);if(marker?.done)return marker;
  const indexed=await indexedItemsAll(s),items=indexed.items||[],bad=items.filter(item=>isV380BadTime(item?.t)),keep=items.filter(item=>!isV380BadTime(item?.t));
  for(const item of bad){try{await retry(()=>s.delete(item.key),120)}catch(e){console.warn('v380-history-delete',item.key,e)}}
  try{await writeFilteredIndexes(s,keep)}catch(e){console.warn('v380-history-reindex',e)}
  const last=keep[keep.length-1]||null;
  if(last){const snap=await safeGet(s,last.key);if(snap?.t&&Array.isArray(snap?.rows))await retry(()=>s.setJSON(LATEST_KEY,{version:2,t:snap.t,fingerprint:snap.fingerprint||fingerprint(snap.rows),key:last.key,count:snap.rows.length}),120)}
  else await retry(()=>s.delete(LATEST_KEY),120).catch(()=>{});
...
async function scrubV391RequestedTimestamp(s){
  const marker=await safeGet(s,V391_CLEANUP_KEY);if(marker?.done)return marker;
  const indexed=await indexedItemsAll(s),items=indexed.items||[],bad=items.filter(item=>isV391BadTime(item?.t)),keep=items.filter(item=>!isV391BadTime(item?.t));
  for(const item of bad){try{await retry(()=>s.delete(item.key),120)}catch(e){console.warn('v391-history-delete',item.key,e)}}
  try{await writeFilteredIndexes(s,keep)}catch(e){console.warn('v391-history-reindex',e)}
  const last=keep[keep.length-1]||null;
  if(last){const snap=await safeGet(s,last.key);if(snap?.t&&Array.isArray(snap?.rows))await retry(()=>s.setJSON(LATEST_KEY,{version:3,t:snap.t,fingerprint:snap.fingerprint||fingerprint(snap.rows),key:last.key,count:snap.rows.length,source:snap.source||null}),120)}
  else await retry(()=>s.delete(LATEST_KEY),120).catch(()=>{});
  const result={done:true,window:['2026-09-09 23:51 EDT','2026-09-09 23:52 EDT'],removed:bad.length,completedAt:new Date().toISOString()};
  await retry(()=>s.setJSON(V391_CLEANUP_KEY,result),120);return result;
}
async function scrubV486PartialConsensus(s){
  const marker=await safeGet(s,V486_CLEANUP_KEY);if(marker?.done)return marker;
  const indexed=await indexedItemsAll(s),items=indexed.items||[],bad=items.filter(item=>isV486BadTime(item?.t)),keep=items.filter(item=>!isV486BadTime(item?.t));
  for(const item of bad){try{await retry(()=>s.delete(item.key),120)}catch(e){console.warn('v486-history-delete',item.key,e)}}
  try{await writeFilteredIndexes(s,keep)}catch(e){console.warn('v486-history-reindex',e)}
  const last=keep[keep.length-1]||null;
  if(last){const snap=await safeGet(s,last.key);if(snap?.t&&Array.isArray(snap?.rows))await retry(()=>s.setJSON(LATEST_KEY,{version:3,t:snap.t,fingerprint:snap.fingerprint||fingerprint(snap.rows),key:last.key,count:snap.rows.length,source:snap.source||null}),120)}
  else await retry(()=>s.delete(LATEST_KEY),120).catch(()=>{});
...
async function scrubV492Post0214History(s){
  const marker=await safeGet(s,V492_CLEANUP_KEY);if(marker?.done)return marker;
  const indexed=await indexedItemsAll(s),items=indexed.items||[],bad=items.filter(item=>isV492BadTime(item?.t)),keep=items.filter(item=>!isV492BadTime(item?.t));
  for(const item of bad){try{await retry(()=>s.delete(item.key),120)}catch(e){console.warn('v492-history-delete',item.key,e)}}
  try{await writeFilteredIndexes(s,keep)}catch(e){console.warn('v492-history-reindex',e)}
  const last=keep[keep.length-1]||null;
  if(last){const snap=await safeGet(s,last.key);if(snap?.t&&Array.isArray(snap?.rows))await retry(()=>s.setJSON(LATEST_KEY,{version:3,t:snap.t,fingerprint:snap.fingerprint||fingerprint(snap.rows,snap.picks||[],snap.teams||[]),key:last.key,count:snap.rows.length,source:snap.source||null}),120)}
  else await retry(()=>s.delete(LATEST_KEY),120).catch(()=>{});
