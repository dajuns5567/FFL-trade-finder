...
function w2Stamp(t,r,key){
  ...
  const voices={
    "walter-mercer":["I think the plain football read is this:","I keep coming back to one thing:","I would underline this part:","I want the result framed this way:","I am less interested in the noise than this:","I see one useful lesson here:","I will give them this:","I am keeping this part of the clipping:"],
    "tess-delaney":["I would put the civilized version this way:","I think this deserves the good china:","I want one elegant point preserved:","I am willing to be pleasant about this:","I would rather not ruin the tablecloth, so:","I see one stylish little truth here:","I will allow myself this indulgence:","I am putting this on the nicer stationery:"],
    "mack-hollis":["I want the loud version on the page:","I am printing this part without apology:","I think rival managers will hate this:","I would put this above the fold:","I am giving this the big headline:","I want every rival manager to see this:","I think the joke is already loaded:","I am keeping this short and loud:"],
    "nora-voss":["I want the funny part first:","I think rivals already see the problem:","I am not dressing this up:","I would put this on the back page too:","I am borrowing Tilly’s volume for this one:","I want this said before the excuses arrive:","I think the joke writes itself here:","I am keeping this simple and mean:"]
  };
  const pool=voices[r?.id]||voices["walter-mercer"],i=(w2Cohort(t)+(w2Hash(key)%8))%8,j=(i+w2Hash(String(r?.id||"")+"|"+String(key||"")))%pool.length;
  return opens[i]+" "+pool[j];
}
...
function rewriteWeek2Overview(overview,teams,previousEdition){
  ...
  const hot=(overview?.hot_takes||[]).map((x,i)=>{
    ...
    if(kind==="championship")return{...x,title:"Week 2 title call: "+subject.team_name,take:w2S(subject,reporter,"hot-champ","two wins are finally enough to make a championship opinion slightly less ridiculous, so I am putting "+subject.team_name+" on the early title line. The point is not certainty; it is that Week 2 gave the opening impression a second result to stand on.")};
    if(kind==="fraud")return{...x,title:"Week 2 danger sign: "+subject.team_name,take:w2S(subject,reporter,"hot-fraud",subject.team_name+" has enough shine to attract confidence and enough Week 2 warning signs to make that confidence expensive. I want another clean Sunday before treating the record as stronger than the problems underneath it.")};
    if(kind==="division"){
      ...
      return{...x,title:"Week 2 division board",take:w2S(subject,reporter,"hot-division","the early division board reads "+flags.map(y=>y.t.team_name+" in "+y.d).join("; ")+". Two weeks are too early for ownership papers and late enough that every team chasing those spots knows exactly who has the first piece of leverage.")};
    }
    if(kind==="player"){
      ...
      return{...x,title:"Week 2 player flag: "+String(p?.name||"the current leader"),take:w2S(pt,reporter,"hot-player",(p?.name||"The current leader")+" has now had two Sundays to turn a first impression into something opponents actually have to plan around. I am backing the player whose role is already making "+pt.team_name+" harder to defend, with plenty of season left to embarrass this prediction.")};
    }
    if(kind==="upset"){
      ...
      return{...x,title:"Week 3 upset call: "+under.team_name+(fav?" over "+fav.team_name:""),take:w2S(under,reporter,"hot-upset",(fav?under.team_name+" over "+fav.team_name:under.team_name+" to steal the next one")+" is the Week 3 call. The projection can keep the favorite label; two weeks of actual football have given me enough reason to pick the messier result before kickoff.")};
    }
