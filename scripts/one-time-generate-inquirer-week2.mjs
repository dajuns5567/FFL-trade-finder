...
function w2BuildSections(t,prev){
  ...
  const outlook=[
    ...
    w2S(t,r,"outlook-two",selfLead&&otherLeaders.length?("The "+String(t.division_context?.division_name||"division")+" lead is shared by "+t.team_name+" and "+w2Natural(otherLeaders.map(x=>x.team_name))+". Week 3 can create the first real separation instead of another shared early table."):("the "+String(t.division_context?.division_name||"division")+" table already gives Week 3 leverage; "+t.team_name+" enters with a "+w2Record(t)+" record and cannot treat the next result like an isolated September experiment.")),
