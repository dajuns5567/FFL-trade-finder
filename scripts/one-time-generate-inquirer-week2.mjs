...
function w2BuildSections(t,prev){
  ...
  const lede=[
    ...
    w2S(t,r,"lede-shape","A "+w2One(margin)+"-point "+(won?"win":"loss")+" tells only part of it. "+(won?t.team_name+" kept getting production from the pieces "+opp+" could not erase":opp+" found an answer more often than "+t.team_name+" did, and the lineup never produced the counterpunch it needed")+"."),
    w2S(t,r,"lede-alias","For "+alias.mascot+", the useful part of Week 2 is what survived contact with a second opponent. One Sunday can lie; two can at least start a pattern.")
  ];
  const players=[];
  for(let i=0;i<3;i++){
    const p=top[i];if(!p)continue;const pp=w2PrevPlayer(prev,p.id),acq=p.acquisition;
    players.push(w2S(t,r,"player-stat-"+i,p.name+" scored "+w2One(p.points)+" with "+w2Stat(p)+". "+(i===0?opp+" never found a way to make "+t.team_name+" win somewhere else":t.team_name+" needed that second source of production so "+opp+" could not treat the matchup like a one-player problem")+"."));
    players.push(w2S(t,r,"player-read-"+i,(pp?(p.name+" had "+w2One(pp.points)+" fantasy points in Week 1, so the Week 2 line "+(Number(p.points)>Number(pp.points)?"raised the volume":"changed the shape of the contribution")+" without erasing what came before; "):"")+p.name+" mattered before the final score made the afternoon look settled"+(acq?"; "+p.name+" arrived by trade"+(acq.season&&acq.week?" in Week "+acq.week+" of "+acq.season:"")+", making this Sunday another part of the return "+t.team_name+" paid for.":".")));
  }
  ...
  const eligible=w2EligibleCool(t),coolNames=eligible.length?eligible.map(p=>p.name):top.filter(Boolean).slice(0,2).map(p=>p.name),cool=[
    ...
    w2S(t,r,"cool-two","The next test is simple: make the Week 3 opponent spend lineup decisions on stopping the same thing. That is when one good Sunday starts looking like a role.")
  ];
  const fs=a.fan_sentiment||{},prevSent=prev?.inquirer_article?.fan_sentiment||{},sentiment=[
    w2S(t,r,"sent-one","The Inquirer mood meter sits at "+String(Number(fs.score)||0)+" under “"+String(fs.title||"Week 2 reaction")+".” Two games are not a season, but nobody has to argue from a one-week sample anymore."),
...
function rewriteWeek2Overview(overview,teams,previousEdition){
  ...
  const blocks=chosen.map((g,i)=>{const r=rep(0)||{},w=g.winner,l=g.loser,wstars=(w.starter_details||[]).slice(0,3),lstar=(l.starter_details||[])[0],prev=previous.get(String(w.roster_id)),paras=[];
    ...
    if(i===0){paras.push(w2S(w,r,"recap-top-stats",wstars.map(w2RecapStat).join("; ")+". That is not one player dragging a lineup across the finish line; it is the kind of top-end scoring that makes a huge total hard to chase."));paras.push(w2S(w,r,"recap-top-impact",(wstars[0]?.name||w.team_name)+" set the pace, and "+l.team_name+" never got the relief that usually comes when a fantasy opponent has only one monster to survive. "+w.team_name+" kept finding another scorer."))}
    else{const names=[wstars[0],wstars[1],lstar].filter(Boolean);paras.push(w2S(w,r,"recap-stats-"+i,names.map(w2RecapStat).join("; ")+". The box score already tells the story: the winner got a difference-maker, and the loser never found enough matching production."));paras.push(w2S(w,r,"recap-impact-"+i,(wstars[0]?.name||w.team_name)+" gave "+w.team_name+" the performance it could build around. "+(lstar?.name||l.team_name)+" answered for "+l.team_name+", but there was not enough behind it to flip the matchup."))}
