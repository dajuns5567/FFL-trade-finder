...
function w2BuildSections(t,prev){
  ...
  const outlook=[
    ...
    later.length?w2S(t,r,"outlook-road","After "+next+", the "+alias.mascot+" see "+w2Natural(later.map(x=>x.team_name))+". Banking Week 3 would lower the pressure on that stretch; dropping it would turn the next game into an early recovery assignment. That order matters because "+next+" can change whether "+later[0].team_name+" arrives as an opportunity to build or a game "+t.team_name+" suddenly needs to repair the record."):w2S(t,r,"outlook-road","The schedule beyond Week 3 is not complete enough for a larger claim, so the next assignment stays simple: beat the team on the page.")
  ];
