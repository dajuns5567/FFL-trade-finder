...
function w2BuildSections(t,prev){
  ...
  const lede=[
    ...
    w2S(t,r,"lede-shape",(won?"The "+alias.mascot+" won by "+w2One(margin)+", but the margin is not the best part: "+t.team_name+" kept getting production from pieces "+opp+" could not erase.":"The "+alias.mascot+" lost by "+w2One(margin)+", and "+opp+" kept finding answers before "+t.team_name+" could build a real counterpunch.")),
    w2S(t,r,"lede-alias","For "+alias.mascot+", Week 2 is the first useful pattern check: one Sunday can lie, but two at least give the start a direction worth arguing about.")
  ];
  const players=[];
  for(let i=0;i<3;i++){
    const p=top[i];if(!p)continue;const pp=w2PrevPlayer(prev,p.id),acq=p.acquisition;
    players.push(w2S(t,r,"player-stat-"+i,p.name+" scored "+w2One(p.points)+" with "+w2Stat(p)+"; "+(i===0?"the "+alias.mascot+" made that the center of the plan, and "+opp+" never forced them somewhere less comfortable":"the "+alias.mascot+" needed that second source of production so "+opp+" could not reduce the matchup to one star")+". "));
    players.push(w2S(t,r,"player-read-"+i,(pp?(p.name+" followed "+w2One(pp.points)+" fantasy points in Week 1 with "+w2One(p.points)+" in Week 2; "):"")+p.name+" mattered to the "+alias.mascot+" before the final score settled"+(acq?"; the trade that brought "+p.name+(acq.season&&acq.week?" in Week "+acq.week+" of "+acq.season:"")+" gets another Sunday added to its return.":".")));
  }
  ...
  const weak=(t.starter_details||[]).slice().sort((x,y)=>Number(x.points)-Number(y.points))[0],hot=[
    w2S(t,r,"hot-one",miss&&gap>0?(miss.starter.name+" gets the uncomfortable Week 2 spotlight for the "+alias.mascot+": a legal alternative existed, and the gap was "+w2One(gap)+" points."):(!won?(weak?.name||t.manager_name)+" gets the uncomfortable Week 2 spotlight because "+t.team_name+" lost and the quietest part of the lineup cannot be waved away.":(weak?.name||t.manager_name)+" gets the warning label even after the win because good results are when weak spots are cheapest to fix.")),
    w2S(t,r,"hot-two",(prev&&weak?(weak.name+" now has two Sundays on the "+alias.mascot+" tape, enough to ask whether the quiet role is becoming a pattern; "):"")+"Week 3 will not settle "+t.team_name+"’s season, but repeating the same weakness would make it much harder to dismiss.")
  ];
  const eligible=w2EligibleCool(t),coolNames=eligible.length?eligible.map(p=>p.name):top.filter(Boolean).slice(0,2).map(p=>p.name),cool=[
    w2S(t,r,"cool-one",(coolNames.length?w2Natural(coolNames):t.team_name)+" "+(coolNames.length>1?"both belong":"belongs")+" on the "+alias.mascot+" Week 2 credit list because the production changed the result, not merely the box score."),
    w2S(t,r,"cool-two",alias.mascot+" earned the Week 2 credit; Week 3 decides whether the role is repeatable, because the next opponent will make a point of taking away what just worked.")
  ];
  const fs=a.fan_sentiment||{},prevSent=prev?.inquirer_article?.fan_sentiment||{},sentiment=[
    w2S(t,r,"sent-one","The "+alias.mascot+" mood meter sits at "+String(Number(fs.score)||0)+" under “"+String(fs.title||"Week 2 reaction")+".” Two games are not a season, but this crowd is no longer arguing from a one-week sample."),
    w2S(t,r,"sent-two",(Number.isFinite(Number(prevSent.score))?"The "+alias.mascot+" moved from "+String(Number(prevSent.score))+" in Week 1 to "+String(Number(fs.score)||0)+" now; ":"")+(won?"the win buys patience, but it also makes the next expectation louder.":"the loss burns patience quickly, and Week 3 arrives before the disappointment has time to cool."))
  ];
  ...
  const outlook=[
    w2S(t,r,"outlook-one","Week 3 brings "+next+", currently "+nrecord+(Number(nctx.standings_rank)?" and No. "+String(nctx.standings_rank)+" overall":"")+", out of "+ndiv+". The "+alias.mascot+" get an opponent with its own two-week problem, not a blank canvas."),
    w2S(t,r,"outlook-two",selfLead&&otherLeaders.length?("The "+String(t.division_context?.division_name||"division")+" lead has "+t.team_name+" and "+w2Natural(otherLeaders.map(x=>x.team_name))+" tied. For "+alias.mascot+", Week 3 is a chance to turn that shared line into actual separation."):("The "+String(t.division_context?.division_name||"division")+" table already gives the "+alias.mascot+" Week 3 leverage; at "+w2Record(t)+", they cannot treat the next result like an isolated September experiment.")),
    w2S(t,r,"outlook-three",nextStar?(nextStar.name+" is the first Week 3 problem for the "+alias.mascot+" after "+w2One(nextStar.points||nextStar.season_avg||0)+" fantasy points in the latest available game. Expect "+next+" to test the same weakness "+t.team_name+" showed in Week 2 until it gets stopped."):("the next opponent will attack the same weakness "+t.team_name+" showed in Week 2 until the lineup proves it can close that door.")),
    later.length?(w2S(t,r,"outlook-road-a",next+" is only the first stop for the "+alias.mascot+"; after that come "+w2Natural(later.map(x=>x.team_name))+".")+" "+w2S(t,r,"outlook-road-b",alias.mascot+" cannot treat the order as decoration; banking the next result changes how much pressure the following games have to carry.")+" "+w2S(t,r,"outlook-road-c","If "+alias.mascot+" wastes Week 3, the same stretch stops offering room to build and starts demanding a recovery win.")):w2S(t,r,"outlook-road","the schedule beyond Week 3 is not complete enough to support a bigger claim, so the useful assignment is simple: beat the team actually on the page.")
  ];
