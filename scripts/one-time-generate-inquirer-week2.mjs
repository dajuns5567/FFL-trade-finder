...
function w2BuildSections(t,prev){
  ...
  const outlook=[
    ...
    w2S(t,r,"outlook-two",selfLead&&otherLeaders.length?("The "+String(t.division_context?.division_name||"division")+" lead has "+t.team_name+" and "+w2Natural(otherLeaders.map(x=>x.team_name))+" tied. For "+alias.mascot+", Week 3 is a chance to turn that shared line into actual separation."):("The "+String(t.division_context?.division_name||"division")+" table already gives Week 3 leverage. At "+w2Record(t)+", the "+alias.mascot+" cannot treat the next result like an isolated September experiment.")),
