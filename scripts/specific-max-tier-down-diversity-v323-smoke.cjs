...
const api=ctx.tradeSpecificMaxValueV279;assert(api?.tierDownCandidatePackages&&api?.diversifyTierDownOutgoing,'V323 helpers unavailable');
...
const out=api.diversifyTierDownOutgoing(rows,60);
const sizes=out.slice(0,30).map(r=>r.give.length),counts={1:0,2:0,3:0};for(const x of sizes)counts[x]++;
assert(counts[2]>0&&counts[3]>0,'multi-asset outgoing packages missing from first 30: '+JSON.stringify(counts));
assert(counts[1]<30,'single-asset outgoing packages dominate all first 30');
console.log('V323 specific-player Maximum Value Tier Down package diversity smoke passed',JSON.stringify(counts));
