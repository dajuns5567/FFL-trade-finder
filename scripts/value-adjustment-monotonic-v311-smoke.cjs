...
const ewWilson=Math.exp(-(53-1)/28),capWilson=385*(.55+.35*ewWilson);assert(plus.aAdj>0&&plus.aAdj<=capWilson+1e-9,'added R3 adjustment must be bounded by recalibrated depth value');
...
assert(sf.aAdj<=4000*.55+1e-9,'premium side already raw-ahead must retain the original V311 depth cap');
