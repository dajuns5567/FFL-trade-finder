...
assert(ui.includes('content:"Open"')&&ui.includes('content:"Close"'),'Trade History toggles must show Open/Close state labels');
assert(!ui.includes("open.hindsight?'▴':'▾'")&&!ui.includes("open.original?'▴':'▾'"),'Trade History toggles must not show arrow glyphs');
assert(ui.includes('vh-history-compare-title')&&ui.includes('Historical value comparison'),'Historical Value Comparison must use Fleeced themed heading text');
...
assert(ui.includes('vh-trade-filter-toolbar')&&ui.includes('text-align:center'),'Trade History filter labels must center over their select bars');
assert(ui.includes('background-position:calc(100% - 17px)')&&ui.includes('appearance:none'),'Trade History select arrows must be inset from the right edge');
assert(ui.includes('.vh-trade-filter-toolbar .vh-trade-reset{height:44px'),'Trade History reset button must align with filter bars');
assert(ui.includes('.vh-view-history{white-space:nowrap;padding:4px 0')&&ui.includes('border:0!important')&&ui.includes('background:transparent!important'),'Value History View history actions must not use pill/oval styling');
assert(ui.includes('.vh-player-link .vh-history-cue{color:#e4b53f!important}'),'Full Market History View history cue must use Fleeced gold');
assert(ui.includes("root.querySelectorAll('.vh-profile-primary h2,.vh-profile-fact small,.vh-profile-fact b"),'Player and fantasy-team text fitting must use local profile-cell selectors');
...
assert(ui.includes("if(firstRows.length<100){scheduleSnapshot(2000,source);return false}")&&ui.includes("if(rows.length<100||!sameSnapshotRows(firstRows,rows)){scheduleSnapshot(2000,source);return false}"),'player rows must be the only required snapshot payload and must be stable before optional enrichment');
assert(ui.indexOf("const rows=currentRows();")<ui.indexOf("try{picks=currentPickRows()}catch"),'stable core player snapshot must be built before optional pick enrichment');
assert(ui.indexOf("const rows=currentRows();")<ui.indexOf("try{teams=currentTeamRows(rows)}catch"),'stable core player snapshot must be built before optional team enrichment');
...
assert(headlessWorkflow.includes('workflow_run:'),'scheduled headless watchdog must also wake from successful GitHub workflow completions');
for(const name of ['Archive Value History','Import Sleeper League History','Sync MIDA team context'])assert(headlessWorkflow.includes(`- ${name}`),`workflow_run wake source missing: ${name}`);
assert(headlessWorkflow.includes("github.event.workflow_run.conclusion == 'success'"),'workflow_run wakeups must fail closed to successful upstream completions only');
assert(headlessWorkflow.includes('branches:')&&headlessWorkflow.includes('- main'),'workflow_run wakeups must be restricted to main');
assert(!headlessWorkflow.includes('workflow_run:\n    workflows:\n      - Scheduled Value History Browser Refresh'),'headless workflow must never recursively wake itself');
