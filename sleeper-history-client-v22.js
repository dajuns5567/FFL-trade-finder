...
function usable22(data){let players=0,withGames=0,withPpr=0;for(const r of Object.values(data||{})){players++;if(games22(r)>0)withGames++;if(Number.isFinite(Number(r?.pts_ppr)))withPpr++}return players>25&&withGames>20&&withPpr>20}
...
loadQualifiedProduction=async function(season){
  // First use the same direct Sleeper season aggregates that the app successfully used before the Netlify history refactor.
  // This path refreshes whenever Update runs and never mutates roster ownership.
