...
function applyHistoryMeta(j,fallback=false){
  state.sleeperHistory={generatedAt:j.generatedAt||new Date().toISOString(),currentSeason:Number(j.currentSeason||j.season)||null,completedWeek:Number(j.completedWeek)||0,weightPlan:j.weightPlan||null,chain:j.chain||[],requiredYears:j.requiredYears||j.years||Object.keys(j.weightPlan?.yearWeights||{}).map(Number),qualifyingHistoricalSeasonMinimumGames:j.qualifyingHistoricalSeasonMinimumGames||8,pprDiagnostics:j.pprDiagnostics||null,pprScoringWeights:j.pprScoringWeights||null,seasonFetchSource:j.seasonFetchSource||null,complete:j.complete!==false,fallback};
}
loadQualifiedProduction=async function(season){
  try{
    ...
    const j=await r.json();if(!j?.ok||!j?.complete||!j?.aggregatedBySeason||!j?.weightPlan)throw Error(j?.error||'history endpoint returned incomplete production data');
    ...
    try{
      ...
      const j=await r.json();if(!j?.ok||!j?.complete||!j?.stats||!j?.weightPlan)throw Error(j?.error||'fallback history endpoint returned incomplete data');
