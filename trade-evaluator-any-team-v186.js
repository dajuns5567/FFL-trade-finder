...
function apply(side){syncRuntime(side);writeDisplay(side)}
...
function onClick(e){if(!on())return;const hit=e.target.closest?.('#evalGlobalResultsA button[data-pid],#evalGlobalResultsB button[data-pid]');if(hit){e.preventDefault();e.stopImmediatePropagation();const side=hit.closest('#evalGlobalResultsA')?'A':'B',a=assetFor(hit.dataset.pid);if(!a)return;stores[side].set(key(a),{...a});const sel=document.getElementById('eval'+side);if(sel&&Number(a.owner))sel.value=String(a.owner);apply(side);const inp=document.getElementById('evalGlobalSearch'+side);if(inp)inp.value='';document.getElementById('evalGlobalResults'+side)?.replaceChildren();return}
 ...
 const btn=e.target.closest?.('button');if(btn&&btn.closest('#evaluator')&&/^(Clear trade|Clear selections)$/i.test((btn.textContent||'').trim())){stores.A.clear();stores.B.clear();for(const side of ['A','B'])apply(side);const box=document.getElementById(CHECK_ID);if(box)box.checked=false;return}}
function onChange(e){if(!on())return;const t=e.target;const side=t?.id==='evalA'?'A':t?.id==='evalB'?'B':null;if(side){setTimeout(()=>writeDisplay(side),0);return}const box=t?.closest?.('input[data-eval-side]');if(box){e.stopImmediatePropagation();const s=box.dataset.evalSide,a=box._asset;if((s==='A'||s==='B')&&a){if(box.checked)stores[s].set(key(a),{...a});else stores[s].delete(key(a));apply(s)}return}}
function install(){ensureControl();if(!document.__evalAnyTeam186){document.__evalAnyTeam186=true;document.addEventListener('click',onClick,true);document.addEventListener('change',onChange,true)}window.__evalAnyTeam179='v186'}
