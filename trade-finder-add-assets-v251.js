...
window.__addAssetsFinderPatchV251=function(src){
  ...
  if(src.split(oldBlank).length-1!==1){console.error('V251 Finder guard failed: blankSelection');return src}
  ...
  if(src.split(oldGives).length-1!==1){console.error('V251 Finder guard failed: manual gives');return src}
  ...
  if(src.split(oldKey).length-1!==1){console.error('V251 Finder guard failed: selected dedupe key');return src}
  ...
  if(src.split(oldBase).length-1!==1){console.error('V251 Finder guard failed: finalize backfill');return src}
  return src.replace(oldBase,newBase);
};

const xhr=new XMLHttpRequest();
xhr.open('GET','/trade-finder-style-loader-v209.js?v=209',false);
try{xhr.send(null)}catch(e){console.error('V251 Finder loader fetch failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V251 Finder loader fetch failed',xhr.status);return}
...
if(loader.split(marker).length-1!==1){console.error('V251 Finder loader guard failed');return}
const inner="try{(0,eval)(src+'\\\n//# sourceURL=trade-finder-v150-v197-runtime.js')}";
const inject="const __v251Inner=\"try{(0,eval)(src+'\\\\\\n//# sourceURL=trade-finder-v150-v197-runtime.js')}\";if(src.split(__v251Inner).length-1!==1){console.error('V251 inner Finder guard failed');return}src=src.replace(__v251Inner,\"src=window.__addAssetsFinderPatchV251(src);\"+__v251Inner);";
loader=loader.replace(marker,inject+marker).replaceAll('V209 Finder','V251 Finder').replace("version:'v209',base:'v208'","version:'v251',base:'v209'");
try{(0,eval)(loader+'\n//# sourceURL=trade-finder-add-assets-v251-loader.js')}catch(e){console.error('V251 Finder loader eval failed',e)}
})();