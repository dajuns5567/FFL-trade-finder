(()=>{
'use strict';
// V193 is built directly from the proven V191 loader. It preserves V191/V190 Tier
// behavior and Maximum Value acceptance/full-pool logic. The only new behavior is
// a neutral/Make a Fair Trade package-variety preference for Maximum Value.
const xhr=new XMLHttpRequest();
xhr.open('GET','/trade-finder-v150.js?v=178',false);
try{xhr.send(null)}catch(e){console.error('V193 Finder load failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V193 Finder load failed',xhr.status);return}
...
if(recommendCount!==2){console.error('V193 Finder guard failed: expected 2 recommendation sites, found',recommendCount);return}
...
if(acceptCount!==2){console.error('V193 Finder guard failed: expected 2 fairness acceptance sites, found',acceptCount);return}
...
if(bestCount!==2){console.error('V193 Finder guard failed: expected 2 candidate comparators, found',bestCount);return}
...
const newBlankShape="shape=(fairAny(tier,w)||tier==='down'||(maximumValueActive()&&tier==='neutral'))?`|${packageShape(recv)}|p${playerCount(recv)}|a${recv.length}`:''";
if(src.split(oldBlankShape).length-1!==1){console.error('V193 Finder guard failed: blank shape signature changed');return}
src=src.replace(oldBlankShape,newBlankShape);
const oldSelectedShape="keepShape=fairAny(tier,w)||give.length>=3";
const newSelectedShape="keepShape=fairAny(tier,w)||give.length>=3||tier==='down'||(maximumValueActive()&&tier==='neutral')";
if(src.split(oldSelectedShape).length-1!==1){console.error('V193 Finder guard failed: selected shape signature changed');return}
...
if(src.split(oldBlankThree).length-1!==1){console.error('V193 Finder guard failed: blank three-asset gate changed');return}
...
if(src.split(oldAllowThree).length-1!==1){console.error('V193 Finder guard failed: selected three-asset gate changed');return}
...
if(src.split(oldTake).length-1!==1){console.error('V193 Finder guard failed: partner package-mix signature changed');return}
...
if(src.split(oldPresentation).length-1!==1){console.error('V193 Finder guard failed: presentationSort signature changed');return}
...
if(src.split(oldBlankSort).length-1!==1){console.error('V193 Finder guard failed: blankDisplaySort signature changed');return}
src=src.replace(oldBlankSort,newBlankSort);

// V193 preserves V191's full eligible Maximum Value pool: no value-tier filtering,
// slicing, capping, or candidate deletion is introduced by the neutral mixer.

const oldNonBlank="if(!blank)return varied.sort(presentationSort);";
const newNonBlank="if(!blank){let ordered=varied.sort(presentationSort);ordered=maximumValueDiversify(ordered);ordered=maximumValueNeutralPackageMix(ordered);return ordered}";
if(src.split(oldNonBlank).length-1!==1){console.error('V193 Finder guard failed: nonblank finalize signature changed');return}
...
const newBlankReturn="const diversified=stabilizeBlankOrder(varied);let ordered=diversified.map((r,i)=>({r,i})).sort((a,b)=>blankDisplaySort(a.r,b.r)||a.i-b.i).map(x=>x.r);ordered=maximumValueDiversify(ordered);ordered=maximumValueNeutralPackageMix(ordered);if(tier==='down')ordered=tierDownGiveMix(ordered);return ordered";
if(src.split(oldBlankReturn).length-1!==1){console.error('V193 Finder guard failed: blank finalize signature changed');return}
src=src.replace(oldBlankReturn,newBlankReturn);

try{(0,eval)(src+'\n//# sourceURL=trade-finder-v150-v193-runtime.js')}catch(e){console.error('V193 Finder eval failed',e);return}
window.__tradeFinderStyleV188={version:'v193',recommendationSites:recommendCount,acceptanceSites:acceptCount,candidateSites:bestCount,base:'v191'};
})();
