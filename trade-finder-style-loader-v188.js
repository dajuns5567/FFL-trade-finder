...
// Tier Down: preserve package shape so valid 2- and 3-asset returns do not collapse.
...
// Tier Down: generate 3-asset returns even when only one outgoing asset is used.
...
// Tier Down: at partner selection, mix valid receive counts so 3-asset returns are
// represented instead of being crowded out by the top-scoring 2-asset versions.
...
if(src.split(oldTake).length-1!==1){console.error('V190 Finder guard failed: partner package-mix signature changed');return}
src=src.replace(oldTake,newTake);

// Maximum Value custom ordering is disabled for Acquire Draft Picks so the frozen
// draft-package presentation logic remains authoritative.
...
if(src.split(oldBlankSort).length-1!==1){console.error('V190 Finder guard failed: blankDisplaySort signature changed');return}
...
// Final presentation-only diversification. Tier Down outgoing-package mixing runs
// last so its package-style precedence is preserved when combined with Maximum Value.
