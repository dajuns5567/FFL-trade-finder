(()=>{
'use strict';
// V190 leaves the frozen V178 Finder source untouched. It loads the same source,
// preserves Best Partner Fit and Maximum Value behavior, and applies isolated
// Tier Down generation/presentation and Maximum Value diversity refinements.
const xhr=new XMLHttpRequest();
xhr.open('GET','/trade-finder-v150.js?v=178',false);
try{xhr.send(null)}catch(e){console.error('V190 Finder load failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V190 Finder load failed',xhr.status);return}
...
if(recommendCount!==2){console.error('V190 Finder guard failed: expected 2 recommendation sites, found',recommendCount);return}
...
// Maximum Value remains the only style with a different fairness acceptance window.
const oldAccept='if(!f||f.rejected)continue;';
const acceptCount=src.split(oldAccept).length-1;
if(acceptCount!==2){console.error('V190 Finder guard failed: expected 2 fairness acceptance sites, found',acceptCount);return}
...
if(bestCount!==2){console.error('V190 Finder guard failed: expected 2 candidate comparators, found',bestCount);return}
src=src.split(oldBest).join('if(!old||candidateBetterV188(r,old))best.set(ck,r)');

// Tier Down: preserve 2- and 3-asset package shapes as separate candidates.
...
if(src.split(oldBlankShape).length-1!==1){console.error('V190 Finder guard failed: blank shape signature changed');return}
...
if(src.split(oldSelectedShape).length-1!==1){console.error('V190 Finder guard failed: selected shape signature changed');return}
src=src.replace(oldSelectedShape,newSelectedShape);

// Tier Down generation fix: the frozen Finder previously generated 3-asset returns
// only when the outgoing side already contained 2+ assets. Tier Down is allowed to
// return 3 assets even for one outgoing asset, so open that gate only for tier=down.
...
if(src.split(oldBlankThree).length-1!==1){console.error('V190 Finder guard failed: blank three-asset gate changed');return}
...
if(src.split(oldAllowThree).length-1!==1){console.error('V190 Finder guard failed: selected three-asset gate changed');return}
src=src.replace(oldAllowThree,newAllowThree);

// Maximum Value custom ordering is disabled when Acquire Draft Picks is selected,
// preserving the frozen draft-package presentation path exactly.
...
if(src.split(oldPresentation).length-1!==1){console.error('V190 Finder guard failed: presentationSort signature changed');return}
...
const newBlankSort="function blankDisplaySort(a,b){if(maximumValuePresentationActive()){const d=(Number(b?.maximumValueScore)||-1)-(Number(a?.maximumValueScore)||-1);if(d)return d;const e=(Number(b?.f?.edgeEffective)||0)-(Number(a?.f?.edgeEffective)||0);if(e)return e}return Math.round(Number(b?.f?.score)||0)-Math.round(Number(a?.recommend)||0)||Math.round(Number(b?.recommend)||0)-Math.round(Number(a?.recommend)||0)}";
// The exact legacy fairness sort must remain intact; use a separately guarded replacement.
const exactBlankSort="function blankDisplaySort(a,b){return Math.round(Number(b?.f?.score)||0)-Math.round(Number(a?.f?.score)||0)||Math.round(Number(b?.recommend)||0)-Math.round(Number(a?.recommend)||0)}";
const guardedBlankSort="function blankDisplaySort(a,b){if(maximumValuePresentationActive()){const d=(Number(b?.maximumValueScore)||-1)-(Number(a?.maximumValueScore)||-1);if(d)return d;const e=(Number(b?.f?.edgeEffective)||0)-(Number(a?.f?.edgeEffective)||0);if(e)return e}return Math.round(Number(b?.f?.score)||0)-Math.round(Number(a?.f?.score)||0)||Math.round(Number(b?.recommend)||0)-Math.round(Number(a?.recommend)||0)}";
if(src.split(exactBlankSort).length-1!==1){console.error('V190 Finder guard failed: blankDisplaySort signature changed');return}
src=src.replace(exactBlankSort,guardedBlankSort);

// Cap low-value Maximum Value candidates only outside the draft-acquisition path.
const oldBase="let base;if(!blank)base=eligible.slice(0,MAX_RESULTS);else base=selectBlankDistribution(eligible);";
const newBase="let base;if(!blank)base=eligible.slice(0,MAX_RESULTS);else base=selectBlankDistribution(eligible);if(maximumValuePresentationActive())base=maximumValueQualityPool(base);";
if(src.split(oldBase).length-1!==1){console.error('V190 Finder guard failed: finalize base signature changed');return}
src=src.replace(oldBase,newBase);

// Final presentation-only diversification. Tier Down package preference is applied
// last so it takes precedence over Maximum Value presentation preferences.
...
if(src.split(oldNonBlank).length-1!==1){console.error('V190 Finder guard failed: nonblank finalize signature changed');return}
...
if(src.split(oldBlankReturn).length-1!==1){console.error('V190 Finder guard failed: blank finalize signature changed');return}
src=src.replace(oldBlankReturn,newBlankReturn);

try{(0,eval)(src+'\n//# sourceURL=trade-finder-v150-v190-runtime.js')}catch(e){console.error('V190 Finder eval failed',e);return}
window.__tradeFinderStyleV188={version:'v190',recommendationSites:recommendCount,acceptanceSites:acceptCount,candidateSites:bestCount};
})();
