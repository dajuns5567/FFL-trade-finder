(()=>{
'use strict';
// V194 is the proven V191 runtime with one isolated Maximum Value + neutral scoring tweak.
// All V191 generation, Tier behavior, 250-result pool, Partner Fit and draft behavior remain unchanged.
const xhr=new XMLHttpRequest();
xhr.open('GET','/trade-finder-v150.js?v=178',false);
try{xhr.send(null)}catch(e){console.error('V194 Finder load failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V194 Finder load failed',xhr.status);return}
...
if(!src.includes(fitFn)){console.error('V194 Finder guard failed: partnerFit signature changed');return}
...
if(recommendCount!==2){console.error('V194 Finder guard failed: expected 2 recommendation sites, found',recommendCount);return}
...
if(acceptCount!==2){console.error('V194 Finder guard failed: expected 2 fairness acceptance sites, found',acceptCount);return}
...
if(bestCount!==2){console.error('V194 Finder guard failed: expected 2 candidate comparators, found',bestCount);return}
...
if(src.split(oldBlankShape).length-1!==1){console.error('V194 Finder guard failed: blank shape signature changed');return}
...
if(src.split(oldSelectedShape).length-1!==1){console.error('V194 Finder guard failed: selected shape signature changed');return}
...
if(src.split(oldBlankThree).length-1!==1){console.error('V194 Finder guard failed: blank three-asset gate changed');return}
...
if(src.split(oldAllowThree).length-1!==1){console.error('V194 Finder guard failed: selected three-asset gate changed');return}
...
if(src.split(oldTake).length-1!==1){console.error('V194 Finder guard failed: partner package-mix signature changed');return}
...
if(src.split(oldPresentation).length-1!==1){console.error('V194 Finder guard failed: presentationSort signature changed');return}
...
if(src.split(oldBlankSort).length-1!==1){console.error('V194 Finder guard failed: blankDisplaySort signature changed');return}
src=src.replace(oldBlankSort,newBlankSort);

// V191 full-pool behavior retained: no value-tier filtering/capping/deletion.
...
if(src.split(oldNonBlank).length-1!==1){console.error('V194 Finder guard failed: nonblank finalize signature changed');return}
...
if(src.split(oldBlankReturn).length-1!==1){console.error('V194 Finder guard failed: blank finalize signature changed');return}
src=src.replace(oldBlankReturn,newBlankReturn);

try{(0,eval)(src+'\n//# sourceURL=trade-finder-v150-v194-runtime.js')}catch(e){console.error('V194 Finder eval failed',e);return}
window.__tradeFinderStyleV188={version:'v194',base:'v191',recommendationSites:recommendCount,acceptanceSites:acceptCount,candidateSites:bestCount};
})();
