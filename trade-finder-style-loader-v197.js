...
try{xhr.send(null)}catch(e){console.error('V197 Finder load failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V197 Finder load failed',xhr.status);return}
...
if(src.split(oldRecommend).length-1!==2){console.error('V197 Finder guard failed: recommendation sites');return}
...
if(src.split(oldAccept).length-1!==2){console.error('V197 Finder guard failed: fairness sites');return}
...
if(src.split(oldBest).length-1!==2){console.error('V197 Finder guard failed: candidate sites');return}
...
if(src.split(oldBlankShape).length-1!==1){console.error('V197 Finder guard failed: blank shape');return}
...
if(src.split(oldSelectedShape).length-1!==1){console.error('V197 Finder guard failed: selected shape');return}
...
if(src.split(oldBlankThree).length-1!==1){console.error('V197 Finder guard failed: blank three gate');return}
...
if(src.split(oldAllowThree).length-1!==1){console.error('V197 Finder guard failed: selected three gate');return}
...
if(src.split(oldTake).length-1!==1){console.error('V197 Finder guard failed: partner mix gate');return}
...
if(src.split(oldBlankGiveTail).length-1!==1){console.error('V197 Finder guard failed: blank give cap');return}
...
if(src.split(oldAvail).length-1!==1||src.split(oldTakeCap).length-1!==1){console.error('V197 Finder guard failed: blank result cap');return}
...
if(src.split(waveDecl).length-1!==1){console.error('V197 Finder guard failed: wave declaration');return}
src=src.replace(waveDecl,"recentGives=[],giveWave={n:1};const available=()=>");
const waveCap="if((giveUse.get(gk)||0)>=5)continue;";
if(src.split(waveCap).length-1!==1){console.error('V197 Finder guard failed: wave cap');return}
src=src.replace(waveCap,"if((giveUse.get(gk)||0)>=Math.min(5,giveWave.n))continue;");
const waveAdvance="if(!r)break;picked.add(r);";
if(src.split(waveAdvance).length-1!==1){console.error('V197 Finder guard failed: wave advance');return}
...
if(src.split(oldPresentation).length-1!==1){console.error('V197 Finder guard failed: presentation sort');return}
...
if(src.split(oldBlankSort).length-1!==1){console.error('V197 Finder guard failed: blank sort');return}
...
if(src.split(oldNonBlank).length-1!==1){console.error('V197 Finder guard failed: nonblank finalize');return}
...
if(src.split(oldBlankReturn).length-1!==1){console.error('V197 Finder guard failed: blank finalize');return}
src=src.replace(oldBlankReturn,newBlankReturn);

try{(0,eval)(src+'\n//# sourceURL=trade-finder-v150-v197-runtime.js')}catch(e){console.error('V197 Finder eval failed',e);return}
window.__tradeFinderStyleV188={version:'v197',base:'v196',maxResults:250,blankGiveLimit:80};
})();
