(()=>{
'use strict';
// V202 is a single source transform over V197, carrying forward working V200 behavior.
// Changes are limited to neutral/fair Balanced and Maximum Value 3-asset generation/retention.
const xhr=new XMLHttpRequest();
xhr.open('GET','/trade-finder-style-loader-v197.js?v=197',false);
try{xhr.send(null)}catch(e){console.error('V202 loader fetch failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V202 loader fetch failed',xhr.status);return}
...
if(src.split(stageMarker).length-1!==1){console.error('V202 guard failed: V197 Finder-source stage');return}
const stagePatch=`
const v202OldPartnerCap="function capBlankPartnerThree(arr){let usedThree=false;return(arr||[]).filter(r=>{if((r.recv?.length||0)!==3)return true;if(usedThree)return false;usedThree=true;return true})}";
const v202NewPartnerCap="function capBlankPartnerThree(arr){const expanded=finderMode()==='neutral'&&(searchStyle()==='balanced'||searchStyle()==='value'),limit=expanded?2:1;let used=0;const pairUse=new Set();return(arr||[]).filter(r=>{if((r.recv?.length||0)!==3)return true;if(used>=limit)return false;if(expanded){const pairs=recvThreePairs(r);if(pairs.some(p=>pairUse.has(p)))return false;for(const p of pairs)pairUse.add(p)}used++;return true})}";
if(src.split(v202OldPartnerCap).length-1!==1){console.error('V202 Finder guard failed: partner three cap');return}
src=src.replace(v202OldPartnerCap,v202NewPartnerCap);
const v202OldShare="function capBlankThreeShare(list){const max3=Math.max(1,Math.floor((list?.length||0)*.25));let used=0;return(list||[]).filter(r=>{if((r.recv?.length||0)!==3)return true;if(used>=max3)return false;used++;return true})}";
const v202NewShare="function capBlankThreeShare(list){const strict=finderMode()==='neutral'&&(searchStyle()==='balanced'||searchStyle()==='value'),ratio=strict?0.35:0.25,max3=Math.max(1,Math.floor((list?.length||0)*ratio));let used=0;const pairUse=new Set();return(list||[]).filter(r=>{if((r.recv?.length||0)!==3)return true;if(used>=max3)return false;if(strict){const pairs=recvThreePairs(r);if(pairs.some(p=>pairUse.has(p)))return false;for(const p of pairs)pairUse.add(p)}used++;return true})}";
if(src.split(v202OldShare).length-1!==1){console.error('V202 Finder guard failed: blank three share');return}
src=src.replace(v202OldShare,v202NewShare);
`;
src=src.replace(stageMarker,stageMarker+stagePatch);

// Carry forward V200/V198 generation behavior and extend the same neutral gate to Balanced.
...
if(gateCount!==3){console.error('V202 guard failed: expected 3 neutral generation gates, found',gateCount);return}
src=src.split(needNeutral).join(neutralModes);

// Best Partner Fit stays at 8, Maximum Value stays at 16, Balanced gets 10.
...
if(src.split(oldBuffer).length-1!==1){console.error('V202 guard failed: neutral three-asset buffer signature changed');return}
...
if(src.split(oldNeutralMix).length-1!==1){console.error('V202 guard failed: Maximum Value neutral shape mix changed');return}
src=src.replace(oldNeutralMix,newNeutralMix);

// Preserve V198 Partner Fit overlap behavior exactly. Maximum Value neutral keeps V200 diversity.
...
if(src.split(oldOverlap).length-1!==1){console.error('V202 guard failed: overlap function changed');return}
src=src.replace(oldOverlap,newOverlap);

src=src.replaceAll("V197 Finder","V202 Finder")
       .replace("version:'v197',base:'v196'","version:'v202',base:'v200'")
       .replace('trade-finder-v150-v197-runtime.js','trade-finder-v150-v202-runtime.js');
try{(0,eval)(src+'\n//# sourceURL=trade-finder-style-loader-v202-transformed.js')}catch(e){console.error('V202 loader eval failed',e)}
})();
