(()=>{
'use strict';
// V207 carries forward V206 behavior and fixes only true 2-player-for-2-player selection frequency.
...
try{xhr.send(null)}catch(e){console.error('V207 loader fetch failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V207 loader fetch failed',xhr.status);return}
...
if(src.split(stageMarker).length-1!==1){console.error('V207 guard failed: V197 Finder-source stage');return}
const stagePatch=`
...
if(src.split(v206OldPartnerCap).length-1!==1){console.error('V207 Finder guard failed: partner three cap');return}
...
if(src.split(v206OldShare).length-1!==1){console.error('V207 Finder guard failed: blank three share');return}
...
if(src.split(v206OldFinalizeBase).length-1!==1){console.error('V207 Finder guard failed: finalize base selection');return}
...
if(gateCount!==3){console.error('V207 guard failed: expected 3 neutral generation gates, found',gateCount);return}
...
if(src.split(oldBuffer).length-1!==1){console.error('V207 guard failed: neutral three-asset buffer signature changed');return}
...
if(src.split(oldNeutralMix).length-1!==1){console.error('V207 guard failed: Maximum Value neutral shape mix changed');return}
...
if(src.split(oldTierUpMix).length-1!==1){console.error('V207 guard failed: Maximum Value Tier Up shape mix changed');return}
...
if(src.split(oldOverlap).length-1!==1){console.error('V207 guard failed: overlap function changed');return}
src=src.replace(oldOverlap,newOverlap);

// V207: keep V206's presentation sprinkle, but first make sure the selected result set contains the intended share when valid candidates exist.
...
const newCandidateFn="function isTrueTwoPlayerForTwo(r){return(r?.give?.length||0)===2&&(r?.recv?.length||0)===2&&playerCount(r.give)===2&&playerCount(r.recv)===2}function twoPlayerForTwoGap(tier){if(tier==='draft')return 0;const style=searchStyle();if(tier==='neutral'&&(style==='balanced'||style==='need'))return 5;if(tier==='down')return 8;return 6}function ensureTwoPlayerForTwoShare(base,eligible,tier){const gap=twoPlayerForTwoGap(tier);if(!gap||!(base||[]).length)return base;const desired=Math.floor(base.length/gap),current=base.filter(isTrueTwoPlayerForTwo).length,need=Math.max(0,desired-current);if(!need)return base;const selected=new Set(base),pool=(eligible||[]).filter(r=>isTrueTwoPlayerForTwo(r)&&!selected.has(r));if(!pool.length)return base;const out=base.slice();let added=0;for(let i=out.length-1;i>=0&&added<need&&pool.length;i--){if(isTrueTwoPlayerForTwo(out[i]))continue;out[i]=pool.shift();added++}return out}function sprinkleTwoPlayerForTwo(list,tier){if(tier==='draft'||!(list||[]).length)return list;const twos=[],rest=[];for(const r of list||[])(isTrueTwoPlayerForTwo(r)?twos:rest).push(r);if(!twos.length||!rest.length)return list;const gap=twoPlayerForTwoGap(tier);const desired=Math.min(twos.length,Math.max(1,Math.floor((list||[]).length/gap))),featured=twos.slice(0,desired),featuredSet=new Set(featured),base=(list||[]).filter(r=>!featuredSet.has(r)),out=[];let bi=0,fi=0,slot=1;while(bi<base.length||fi<featured.length){if(fi<featured.length&&slot%gap===0)out.push(featured[fi++]);else if(bi<base.length)out.push(base[bi++]);else if(fi<featured.length)out.push(featured[fi++]);slot++}return out.length===(list||[]).length?out:list}function candidateBetterV188(a,b){if(maximumValueActive()){const avs=Number(a?.maximumValueScore)||-1,bvs=Number(b?.maximumValueScore)||-1;if(avs!==bvs)return avs>bvs;const ae=Number(a?.f?.edgeEffective)||0,be=Number(b?.f?.edgeEffective)||0;if(ae!==be)return ae>be}return a.recommend>b.recommend||(a.recommend===b.recommend&&a.gap<b.gap)}";
if(src.split(oldCandidateFn).length-1!==1){console.error('V207 guard failed: candidate helper signature changed');return}
...
if(finalTailCount!==2){console.error('V207 guard failed: expected 2 final ordering tails, found',finalTailCount);return}
...
src=src.replaceAll("V197 Finder","V207 Finder")
       .replace("version:'v197',base:'v196'","version:'v207',base:'v206'")
       .replace('trade-finder-v150-v197-runtime.js','trade-finder-v150-v207-runtime.js');
try{(0,eval)(src+'\n//# sourceURL=trade-finder-style-loader-v207-transformed.js')}catch(e){console.error('V207 loader eval failed',e)}
})();
