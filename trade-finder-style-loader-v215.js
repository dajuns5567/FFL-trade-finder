(()=>{
'use strict';
// V215 starts from frozen V209. Future-Oriented is isolated to findMode=rebuild and is disabled for Acquire Draft Picks.
// It changes candidate preference inside existing structure buckets; V209 still owns generation, eligibility, fairness,
// package construction, structure mixing, 2-for-2 behavior, 3-asset spacing, Partner Fit, Maximum Value, and Evaluator logic.
const xhr=new XMLHttpRequest();
xhr.open('GET','/trade-finder-style-loader-v209.js?v=209',false);
try{xhr.send(null)}catch(e){console.error('V215 loader fetch failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V215 loader fetch failed',xhr.status);return}
...
if(src.split(blankOld).length-1!==1){console.error('V215 guard failed: blank shape preservation');return}
...
if(src.split(selectedOld).length-1!==1){console.error('V215 guard failed: selected shape preservation');return}
...
if(src.split(takeOld).length-1!==1){console.error('V215 guard failed: partner candidate slice');return}
...
if(src.split(betterOld).length-1!==1){console.error('V215 guard failed: candidate comparator');return}
src=src.replace(betterOld,betterNew);

// Patch the frozen Finder comparator before V197/V209 transforms run. This is ranking only; no eligibility/value mutation.
const stageOld="const stagePatch=`\n";
const stageNew=`const stagePatch=\`\nconst v215OldPresentationSort=\"function presentationSort(a,b){return b.recommend-a.recommend||b.f.score-a.f.score||a.gap-b.gap}\";\nconst v215NewPresentationSort=\"function futureActive(){return searchStyle()==='rebuild'&&finderMode()!=='draft'}function futureQuality(x){return clamp(0,(av(x)-900)/5000,1)}function futureAgeScore(x){if(x?.type!=='player')return 50;const a=Number(st().players?.[id(x)]?.age);if(!Number.isFinite(a))return 50;const p=pos(x),peak=p==='RB'?24:p==='WR'?26:p==='QB'?29:p==='TE'?27:27,late=p==='RB'?10:p==='WR'?7:p==='QB'?4:p==='TE'?6:5;return clamp(10,100-Math.max(0,a-peak)*late,100)}function futureAssetScore(x){if(x?.type==='pick'){const q=clamp(0,(av(x)-350)/3600,1),r=Number(x?.round)||4,liq=r===1?15:r===2?9:r===3?4:0;return clamp(0,q*85+liq,100)}const q=futureQuality(x),age=futureAgeScore(x);return clamp(0,q*75+q*age*.25,100)}function futurePackageScore(xs){let n=0,d=0;for(const x of xs||[]){const w=Math.max(x?.type==='pick'?500:250,av(x));n+=futureAssetScore(x)*w;d+=w}return d?n/d:50}function futurePickScore(xs){const ps=(xs||[]).filter(x=>x?.type==='pick');return ps.length?ps.reduce((s,x)=>s+futureAssetScore(x),0)/ps.length:50}function futureTradeScore(r){const recv=futurePackageScore(r.recv),give=futurePackageScore(r.give),delta=clamp(0,50+(recv-give),100),tier=finderMode();if(tier==='up'){const c=rankCenter(r.recv),center=c?futureAssetScore(c):50;return clamp(0,center*.60+recv*.25+delta*.15,100)}if(tier==='down')return clamp(0,recv*.45+futurePickScore(r.recv)*.25+delta*.30,100);return clamp(0,50+(recv-give)*.55+(recv-50)*.25+(futurePickScore(r.recv)-50)*.20,100)}function futureComposite(r){if(!futureActive())return Number(r?.recommend)||0;const tier=finderMode(),w=tier==='up'?.30:tier==='down'?.28:.25;return (Number(r?.recommend)||0)*(1-w)+futureTradeScore(r)*w}function presentationSort(a,b){if(futureActive()){const d=futureComposite(b)-futureComposite(a);if(d)return d}return b.recommend-a.recommend||b.f.score-a.f.score||a.gap-b.gap}\";\nif(src.split(v215OldPresentationSort).length-1!==1){console.error('V215 Finder guard failed: presentationSort');return}\nsrc=src.replace(v215OldPresentationSort,v215NewPresentationSort);\n`;
if(src.split(stageOld).length-1!==1){console.error('V215 guard failed: Finder-source stage');return}
src=src.replace(stageOld,stageNew)
       .replaceAll('V209 Finder','V215 Finder')
       .replace("version:'v209',base:'v208'","version:'v215',base:'v209'")
       .replace('trade-finder-v150-v209-runtime.js','trade-finder-v150-v215-runtime.js')
       .replace('trade-finder-style-loader-v209-transformed.js','trade-finder-style-loader-v215-transformed.js');
try{(0,eval)(src+'\n//# sourceURL=trade-finder-style-loader-v215-wrapper.js')}catch(e){console.error('V215 loader eval failed',e)}
})();
