...
function pickPackages(picks,target,light=false){
...
function tierOK(give,recv,tier){
 ...
 const c=centerpiece(recv);if(targetPos()!=='ANY'&&(!c||pos(c)!==targetPos()))return false;
 if(tier==='neutral')return true;
 const gp=give.filter(x=>x.type==='player');
 if(tier==='up'){
   if(!c)return false;
   if(gp.length){const bestGive=Math.min(...gp.map(rankOf));return rankOf(c)<bestGive}
   return av(c)>Math.max(...give.map(av),0);
 }
 if(tier==='down'){
   if(!gp.length||recv.length<2||!c)return false;
   const bestGive=Math.min(...gp.map(rankOf));return rankOf(c)>bestGive;
 }
 return true;
}
...
function generate(){
 ...
 for(const give of givePkgs){
   ...
   for(const tm of st().teams||[]){
     ...
     const incoming=candidateIncoming(owned,target,tier,blank);
     ...
     for(const recv of incoming){
       ...
       const c=centerpiece(recv),centerKey=c?`player:${id(c)}`:`picks:${assetKey(recv)}`;
...
function rationale(r){const mode=finderMode(),style=searchStyle(),bits=[];bits.push(`The packages pass the current Value Adjustment-only fairness model at ${Math.round(r.f.score)}/100.`);bits.push(`${teamName(r.other)} is a current partner match under the ${style.replace(/-/g,' ')} search style (partner-fit ${Math.round(r.fit)}/100).`);if(targetPos()!=='ANY')bits.push(`The incoming centerpiece is a ${targetPos()}, matching the selected position filter.`);if(mode==='up')bits.push('Tier up requires the incoming centerpiece to rank higher than the best outgoing player.');else if(mode==='down')bits.push('Tier down requires a lower-ranked incoming centerpiece plus additional package value.');else if(mode==='draft')bits.push('Acquire draft picks returns pick-only incoming packages and obeys the selected year/round filters.');else bits.push('Make a fair trade prioritizes fairness and partner fit without forcing a tier direction.');return `<ul>${bits.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`}
