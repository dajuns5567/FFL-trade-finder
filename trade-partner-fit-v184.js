...
function rebuildCache(){
 ...
 for(const t of s.teams||[]){const team=Number(t.id);for(const p of positions){const a=(grouped.get(`${team}|${p}`)||[]).slice().sort((x,y)=>val(y)-val(x)),n=slots[p]||1;let sum=0,w=0;for(let i=0;i<Math.min(a.length,n+2);i++){const wt=i<n?1:(i===n?.42:.2);sum+=val(a[i])*wt;w+=wt}rooms.set(`${team}|${p}`,w?sum/w:0)}}
...
window.tradePartnerFitV184={active,comprehensive,recommendation,invalidate,version:'v185'};
})();
