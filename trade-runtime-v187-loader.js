...
const newAdjustment="function adjustment(give,recv){const ar=raw(give),br=raw(recv),aTop=(give||[]).slice().sort((x,y)=>av(y)-av(x))[0]||null,bTop=(recv||[]).slice().sort((x,y)=>av(y)-av(x))[0]||null,am=aTop?av(aTop):0,bm=bTop?av(bTop):0;let aa=0,ba=0;const amp=asset=>{if(!asset||asset.type!=='player')return 1.10;const r=Math.max(1,rankOf(asset));return 1.10+.44*Math.exp(-(r-1)/35)};const eliteWeight=asset=>asset?.type==='player'?Math.exp(-(Math.max(1,rankOf(asset))-1)/28):0;const calc=(premium,otherTop,otherRaw,premiumRaw,asset)=>{if(premium<=otherTop||otherTop<=0)return 0;const depth=Math.max(0,otherRaw-otherTop);if(depth<=0)return 0;const rel=clamp(0,otherTop/premium,1),strength=premium/(premium+3500),rate=.075+.18*strength,counter=1-.75*Math.pow(rel,1.4),disp=1+.8*(1-rel),elite=1.3+1.5*clamp(0,(premium-5000)/5000,1),smooth=1+.39*clamp(0,(8800-premium)/1890,1),legacy=premium*rate*counter*disp*elite*smooth*amp(asset),rawGap=Math.max(0,otherRaw-premiumRaw),ew=eliteWeight(asset),gapTarget=rawGap>0?rawGap*(.50+.50*ew):0,depthCap=depth*(.55+(rawGap>0?.35*ew:0));return Math.min(Math.max(legacy,gapTarget),depthCap)};if(am>bm)aa=calc(am,bm,br,ar,aTop);else if(bm>am)ba=calc(bm,am,ar,br,bTop);return{aRaw:ar,bRaw:br,aAdj:aa,bAdj:ba}}";
