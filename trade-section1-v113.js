...
function installLogo(){
  ...
  h.className='fleecedFlat113';h.setAttribute('aria-label','Fleeced!');
  if(!h.querySelector('.fleecedWord113'))h.innerHTML='<span class="fleecedWord113">Fleeced!</span><span class="fleecedArrow113" aria-hidden="true"></span>';
...
function parseSide(side){
  let raw=0,penalty=0,players=0,picks=0,topPlayer=0,bestRank=9999;const names=[],positions=[];
  side.querySelectorAll('.trade95-asset').forEach(row=>{
    ...
    const txt=row.textContent||'',m=txt.match(/overall\s*#\s*(\d+)/i),pm=txt.match(/\b(QB|RB|WR|TE|IDP)\b/i);if(m){players++;const r=Number(m[1]);bestRank=Math.min(bestRank,r);topPlayer=Math.max(topPlayer,v);if(r>=350)positions.push({v,rank:r})}else if(/\b20\d{2}\s+R[123]\b/i.test(txt))picks++;
    const nm=row.querySelector('b')?.textContent?.trim();if(nm)names.push(nm);
  });
  if(positions.length>=2){const frag=Math.min(1.46,1+.28*Math.max(0,positions.length-2));for(const x of positions)penalty+=x.v*Math.min(.58,lowTierRate(x.rank)*frag);penalty=Math.min(raw*.64,penalty)}
  const adjRow=[...side.querySelectorAll('.trade97-adjust,.trade95-adjust,div')].find(x=>/VALUE ADJUSTMENT/i.test(x.textContent||''));let adj=0;if(adjRow){const m=(adjRow.textContent||'').match(/\+\s*([\d,.]+)/);if(m)adj=Number(m[1].replace(/,/g,''))||0}
  if(penalty>0)adj=0;
...
function showPenalty(side,d){
  side.querySelectorAll('.packagePenalty113,.packageAfter113').forEach(x=>x.remove());if(!d.penalty)return;
  const total=side.querySelector('.trade95-total');if(!total)return;
  const p=document.createElement('div');p.className='trade97-adjust packagePenalty113';p.innerHTML=`<span>PACKAGE QUALITY PENALTY</span><b>−${d.penalty.toLocaleString(undefined,{maximumFractionDigits:1})}</b>`;
  const a=document.createElement('div');a.className='trade97-effective packageAfter113';a.innerHTML=`<span>AFTER PACKAGE PENALTY</span><b>${Math.max(0,d.raw-d.penalty).toLocaleString(undefined,{maximumFractionDigits:1})}</b>`;
  total.insertAdjacentElement('afterend',p);p.insertAdjacentElement('afterend',a);
}
...
function rewriteRationale(card,recv,send){
  ...
  const li=body.querySelector('li');if(li)li.textContent=reason;else{const n=document.createElement('li');n.textContent=reason;body.prepend(n)}
}
function postProcessFinder(){
  const host=document.getElementById('finderResults');if(!host)return;const tier=document.getElementById('tradeTier94')?.value||'neutral';
  ...
  for(const card of cards){const sides=card.querySelectorAll('.trade95-side');if(sides.length<2)continue;const recv=parseSide(sides[0]),send=parseSide(sides[1]);showPenalty(sides[0],recv);showPenalty(sides[1],send);
    let hi=Math.max(recv.effective,send.effective,1),lo=Math.min(recv.effective,send.effective),score=Math.round(clamp(1,100-(Math.abs(recv.effective-send.effective)/hi)*150,100));
    ...
    const lowPackage=(recv.penalty>0||send.penalty>0),summary=card.querySelector('.trade95-summary>div');if(summary){summary.querySelectorAll('.packagePenaltySummary113').forEach(x=>x.remove());if(lowPackage){const b=document.createElement('strong');b.className='packagePenaltySummary113';b.textContent=`Package penalty −${(recv.penalty+send.penalty).toLocaleString(undefined,{maximumFractionDigits:1})}`;summary.appendChild(b)}}
    const scoreBox=card.querySelector('.trade95-score');if(scoreBox){const span=scoreBox.querySelector('span'),label=scoreBox.querySelector('div');scoreBox.childNodes.forEach(n=>{if(n.nodeType===3)n.nodeValue=''});scoreBox.insertBefore(document.createTextNode(String(score)),span||scoreBox.firstChild);if(label)label.textContent=score>=94?'Excellent Fit':score>=82?'Fair':score>=72?'Negotiable':'Not recommended'}
    rewriteRationale(card,recv,send);
    if(score<72)card.remove();
  }
  if(tier==='draft'){
    [...host.querySelectorAll('.trade95-card')].forEach(card=>{const sides=card.querySelectorAll('.trade95-side');if(sides.length<2)return;const recv=parseSide(sides[0]),send=parseSide(sides[1]);if(send.penalty>0&&send.bestRank>=400&&recv.picks>0&&/\b20\d{2}\s+R1\b/i.test(sides[0].textContent||''))card.remove()});
  }
}
function observe(){const host=document.getElementById('finderResults');if(host&&!host.__v113){host.__v113=true;new MutationObserver(()=>setTimeout(postProcessFinder,0)).observe(host,{childList:true,subtree:true})}}
