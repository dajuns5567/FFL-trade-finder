...
let allSelectedTeam='';let applying=false;
...
function applyAlias(){if(!aliasActive()||applying)return false;const boxes=shopBoxes();if(!boxes.length)return false;applying=true;try{for(const box of boxes){if(box.checked)continue;box.checked=true;box.setAttribute('checked','');box.dispatchEvent(new Event('input',{bubbles:true}));box.dispatchEvent(new Event('change',{bubbles:true}))}syncButton();return true}finally{applying=false}}
...
function selectAllAnchor(team){const col=team?.parentElement;if(!col)return team;const search=[...col.querySelectorAll('input')].find(x=>/search player/i.test(String(x.placeholder||''))||/search any (league )?player/i.test(String(x.placeholder||'')));return search||team}
function ensureButton(){const team=document.getElementById(TEAM_ID);if(!team)return;const selected=currentTeam();let wrap=document.getElementById(WRAP_ID);if(!selected){wrap?.remove();return}if(allSelectedTeam&&allSelectedTeam!==selected)allSelectedTeam='';if(!wrap){wrap=document.createElement('div');wrap.id=WRAP_ID;wrap.style.cssText='margin-top:8px';const btn=document.createElement('button');btn.type='button';btn.id='tradeSelectAllButton165';btn.className='secondary';btn.style.cssText='width:100%;font-weight:700';btn.textContent='Select all';btn.addEventListener('click',selectAllBlankAlias);wrap.appendChild(btn)}const anchor=selectAllAnchor(team);if(anchor&&anchor.nextElementSibling!==wrap)anchor.insertAdjacentElement('afterend',wrap);if(!applyAlias())syncButton()}
