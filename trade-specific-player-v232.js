...
function outgoing(me,targetValue){
 ...
 const nearPlayers=[...players].sort((a,b)=>Math.abs(av(a)-targetValue)-Math.abs(av(b)-targetValue)||av(b)-av(a)).slice(0,14);
 const premiumPlayers=[...players].sort((a,b)=>av(b)-av(a)||rank(a)-rank(b)).slice(0,8);
 const rankedPlayers=[...players].sort((a,b)=>rank(a)-rank(b)||av(b)-av(a)).slice(0,8);
 const comboPlayers=uniqAssets([...nearPlayers,...premiumPlayers,...rankedPlayers]).slice(0,20);
 const triplePlayers=comboPlayers.slice(0,14);
 const nearPicks=[...picks].sort((a,b)=>Math.abs(av(a)-targetValue)-Math.abs(av(b)-targetValue)||av(b)-av(a)).slice(0,8);
 const oneP=closestPackages(nearPlayers.map(x=>[x]),targetValue,12),oneK=closestPackages(nearPicks.map(x=>[x]),targetValue,6);
 const allPairs=[],pk=[],kk=[],ppp=[],ppk=[],pkk=[],kkk=[];
 for(let i=0;i<players.length;i++)for(let j=i+1;j<players.length;j++)allPairs.push([players[i],players[j]]);
 const pp=uniq([
   ...closestPackages(allPairs,targetValue,14),
   ...closestPackages(allPairs,targetValue*1.25,10),
   ...closestPackages(allPairs,targetValue*1.5,8),
   ...closestPackages(allPairs,targetValue*1.75,6)
 ]);
 for(const p of comboPlayers)for(const k of nearPicks)pk.push([p,k]);
 for(let i=0;i<nearPicks.length;i++)for(let j=i+1;j<nearPicks.length;j++)kk.push([nearPicks[i],nearPicks[j]]);
 for(let i=0;i<triplePlayers.length;i++)for(let j=i+1;j<triplePlayers.length;j++)for(let k=j+1;k<triplePlayers.length;k++)ppp.push([triplePlayers[i],triplePlayers[j],triplePlayers[k]]);
 for(let i=0;i<triplePlayers.length;i++)for(let j=i+1;j<triplePlayers.length;j++)for(const k of nearPicks.slice(0,6))ppk.push([triplePlayers[i],triplePlayers[j],k]);
 for(const p of triplePlayers)for(let i=0;i<Math.min(6,nearPicks.length);i++)for(let j=i+1;j<Math.min(7,nearPicks.length);j++)pkk.push([p,nearPicks[i],nearPicks[j]]);
 for(let i=0;i<Math.min(6,nearPicks.length);i++)for(let j=i+1;j<Math.min(7,nearPicks.length);j++)for(let k=j+1;k<Math.min(8,nearPicks.length);k++)kkk.push([nearPicks[i],nearPicks[j],nearPicks[k]]);
 const groups={oneP,oneK,pp:pp.slice(0,28),pk:closestPackages(pk,targetValue,18),kk:closestPackages(kk,targetValue,8),ppp:closestPackages(ppp,targetValue,14),ppk:closestPackages(ppk,targetValue,14),pkk:closestPackages(pkk,targetValue,10),kkk:closestPackages(kkk,targetValue,8)};
 return mixShapes(groups,['oneP','pp','ppk','oneK','pk','ppp','kk','pkk','oneP','pp','kkk','pk','ppk'],84);
}
function incoming(owner,target,targetValue){
 ...
 const gapPlayers=[...players].sort((a,b)=>Math.abs(av(a)-need)-Math.abs(av(b)-need)||rank(a)-rank(b)).slice(0,12);
 const premiumValue=[...players].sort((a,b)=>av(b)-av(a)||rank(a)-rank(b)).slice(0,6);
 const premiumRank=[...players].sort((a,b)=>rank(a)-rank(b)||av(b)-av(a)).slice(0,6);
 const ps=uniqAssets([...gapPlayers.slice(0,6),...premiumValue,...premiumRank,...gapPlayers.slice(6)]).slice(0,18);
 const ks=[...picks].sort((a,b)=>Math.abs(av(a)-need)-Math.abs(av(b)-need)||av(b)-av(a)).slice(0,8),out=[[target]];
 for(const p of ps)out.push([target,p]);
 for(const k of ks)out.push([target,k]);
 for(const p of ps.slice(0,10))for(const k of ks.slice(0,5))out.push([target,p,k]);
 for(let i=0;i<Math.min(8,ps.length);i++)for(let j=i+1;j<Math.min(9,ps.length);j++)out.push([target,ps[i],ps[j]]);
 return uniq(out).slice(0,90);
}
...
document.addEventListener('click',e=>{const b=e.target.closest?.('#runFinder');if(!b||!targetText())return;e.preventDefault();e.stopImmediatePropagation();run().catch(err=>{console.error('V236 specific-player search failed',err);const host=document.getElementById('finderResults');if(host)host.innerHTML='<div class="empty">Specific-player search stopped unexpectedly. Clear the player field to use the normal Finder.</div>'})},true);
document.addEventListener('click',e=>{const b=e.target.closest?.('button');if(!b||!/^Clear selections$/i.test((b.textContent||'').trim()))return;const input=document.getElementById('desiredPlayerSearch');if(!input||!input.value)return;input.value='';input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}))},true);
