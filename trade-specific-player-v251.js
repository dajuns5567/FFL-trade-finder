...
try{xhr.send(null)}catch(e){console.error('V251 targeted loader fetch failed',e);return}
if(xhr.status<200||xhr.status>=300){console.error('V251 targeted loader fetch failed',xhr.status);return}
let src=xhr.responseText;
const marker='function assetRow(x){';
if(src.split(marker).length-1!==1){console.error('V251 targeted guard failed: assetRow marker');return}
...
if(src.split(oldCall).length-1!==1){console.error('V251 targeted guard failed: retention call');return}
src=src.replace(oldCall,'backfillManualAddAssets(rows,manualCount,250)');
src=src.replace("console.error('V244 specific-player search failed'","console.error('V251 specific-player search failed'");
try{(0,eval)(src+'\n//# sourceURL=trade-specific-player-v251-runtime.js')}catch(e){console.error('V251 targeted eval failed',e)}
})();