(()=>{
'use strict';
const MIN=120,MAX=9999,PLAYER_BREAK=325,PLAYER_BREAK_VALUE=1825,ELITE_FIRST=6500,PICK_MAX=7000,YEAR_DISCOUNT=.88;
...
function documentRows(){
  ...
  if(rows.length!==32)return new Map();rows.sort((a,b)=>b.score-a.score||a.rank-b.rank||a.rid-b.rid);const out=new Map();rows.forEach((r,i)=>out.set(r.rid,{...r,documentRank:i+1,documentSlot:32-i}));return out;
}
...
function projectedSlot(a){
  const rid=originalRoster(a),doc=documentRows().get(rid);if(doc)return doc.documentSlot;
  try{const c=window.draftPickContext86,slot=Number(c?.slots?.get?.(rid));if(Number.isFinite(slot))return clamp(1,slot,32)}catch(_){ }
  return 16;
}
...
function pickValue(a){return round5(clamp(MIN,rawPickValue(a)*(ELITE_FIRST/anchorRaw()),PICK_MAX))}
function pickContext(a){const rid=originalRoster(a),doc=documentRows().get(rid),slot=projectedSlot(a);return{originalRoster:rid,originalTeam:teamName(rid),currentOwner:Number(a?.owner)||0,currentOwnerTeam:teamName(Number(a?.owner)),projectedSlot:slot,value:rawPickValue(a),projectionContextUsed:Boolean(doc),projectionDocumentRank:doc?.documentRank||null,projectionTeamRank:doc?.rank||null,projectionPlayoff:doc?.playoff??null,projectionTitle:doc?.title??null,source:doc?'100% live projection document: 50% projected standings + 35% playoff odds + 15% normalized title odds':'Sleeper projection fallback'}}
function stopLegacyPollers(){for(const k of ['__v124ValuePoll','__v125ValuePoll','__v126ValuePoll','__v127ValuePoll'])if(window[k]){clearInterval(window[k]);window[k]=null}}
function install(){stopLegacyPollers();for(const e of engines()){if(!e.__v128BaseAssetValue){Object.defineProperty(e,'__v128BaseAssetValue',{configurable:true,enumerable:false,writable:false,value:e.__v124BaseAssetValue||e.__v127BaseAssetValue||e.assetValue?.bind(e)})}const base=e.__v128BaseAssetValue;if(typeof base!=='function')continue;const fn=a=>{if(a?.type==='player'){const v=playerValue(a);return v==null?(Number(base(a))||0):v}if(a?.type==='pick')return pickValue(a);return Number(base(a))||0};try{Object.defineProperty(e,'assetValue',{configurable:true,enumerable:true,writable:true,value:fn})}catch(_){e.assetValue=fn}}
  window.pickValue=a=>a?.type==='pick'?pickValue(a):0;window.__tradeValueNormalization='v128-deterministic-10k-source-picks';return true}
setTimeout(install,0);setTimeout(install,150);setTimeout(install,600);if(!window.__v128ValuePoll)window.__v128ValuePoll=setInterval(install,1200);
window.tradeValueNormalizationV128={MIN,MAX,ELITE_FIRST,PICK_MAX,YEAR_DISCOUNT,rankOf,currentMaxRank,playerValueForRank,playerValue,nearestSeason,projectedSlot,rawPickValue,pickValue,pickContext,install};
})();