...
function sourcePickFn(){const f=window.pickValue;return typeof f==='function'?f.bind(window):null}
...
function sourceValue(a){if(!a||a.type!=='pick')return 0;try{const v=Number(sourcePickFn()?.(a));if(Number.isFinite(v)&&v>0)return v}catch(_){}try{const p=sourceProjectionFn();const v=Number(p?.(a)?.value);if(Number.isFinite(v)&&v>0)return v}catch(_){}return 0}
...
function install(){
 ...
 window.__tradeValueNormalization='v135-single-writer-source-pick-scale-7000';return true;
}
install();
setTimeout(install,150);setTimeout(install,700);
window.tradeValueNormalizationV130={MIN,MAX,ELITE_FIRST,rankOf,currentMaxRank,playerValueForRank,playerValue,nearestSeason,sourceValue,sourceAnchor,pickScale,pickValue,pickContext,canonicalValue,install,originalBaseValue,originalPackageValue};
})();