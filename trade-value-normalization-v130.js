...
const engines=()=>[window.tradeEngine96,window.tradeEngine98,window.tradeEngine99].filter(Boolean);
...
function projectedSlot(a){
 const rid=originalRoster(a),doc=projectionRows().get(rid);if(doc)return doc.documentSlot;
 try{const slot=Number(window.draftPickContext86?.slots?.get?.(rid));if(Number.isFinite(slot))return clamp(1,slot,32)}catch(_){ }
 return 16;
}
...
function install(){
 ...
 window.__tradeValueNormalization='v130-single-owner';
 return true;
}
setTimeout(install,0);setTimeout(install,120);setTimeout(install,500);
window.tradeValueNormalizationV130={MIN,MAX,ELITE_FIRST,PICK_MAX,YEAR_DISCOUNT,rankOf,currentMaxRank,playerValueForRank,playerValue,nearestSeason,projectedSlot,rawPickValue,pickValue,pickContext,install};
})();