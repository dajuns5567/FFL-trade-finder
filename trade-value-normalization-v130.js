...
const sourcePickValue=typeof window.pickValue==='function'?window.pickValue.bind(window):null;
const sourcePickProjection=window.draftPickProjection92||window.draftPickProjection90||window.draftPickProjection86||null;
...
function sourceValue(a){if(!a||a.type!=='pick')return 0;try{const v=Number(sourcePickValue?.(a));if(Number.isFinite(v)&&v>0)return v}catch(_){}try{const v=Number(sourcePickProjection?.(a)?.value);if(Number.isFinite(v)&&v>0)return v}catch(_){}return 0}
