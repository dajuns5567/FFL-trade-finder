...
function tierDownDeconsolidationSignal(recv){
 const ps=players(recv),ks=picks(recv),scores=ps.map(playerWinNow).sort((a,b)=>b-a);
 if(ps.length>=2){
  const second=scores[1]||0,third=scores[2]||0;
  let score=second>=55?8.8:second>=42?7.8:second>=30?6:-3;
  if(ps.length>=3)score+=third>=42?1:third>=30?.6:0;
  // Player-only packages are the default Win-Now Tier Down construction.
  // One balancing pick is still allowed, but should remain secondary rather than dominate deeper results.
  if(ks.length===0)score+=1.2;
  else if(ks.length===1)score-=1.6;
  else score-=4.5;
  return clamp(-6,score,10);
 }
 // One player + pick remains legal only as a fallback when stronger player-package alternatives are unavailable.
 if(ps.length===1&&ks.length>=1){
  const best=scores[0]||0;
  return best>=82?-6.5:best>=68?-8.2:-10;
 }
 return ps.length===1?-4:0;
}
...
window.teamContextTradeFit90=function(me,other,style,give,recv){
 ...
 try{
  ...
  if(tier==='down'){
   ...
   return clamp(-10,base*.20+signal*.25+decon*.55,10);
  }
  // Exact V224/V225 behavior for Win-Now + Fair Trade / Tier Up.
  return clamp(-10,base*.38+signal*.62,10);
 }catch(_){return base}
};
window.tradeWinNowPreferencesV226={ageOf,qualityScore,careerWindowScore,playerWinNow,packagePlayerSignal,pickCompositionSignal,tierDownDeconsolidationSignal,winNowSignal};
})();
