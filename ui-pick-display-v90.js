...
function canonicalPickDisplayValue90(x,p){
  try{
    const v=Number(window.tradeValueNormalizationV130?.pickValue?.(x));
    if(Number.isFinite(v)&&v>0)return v;
  }catch(_){}
  const fallback=Number(p?.value);
  return Number.isFinite(fallback)?fallback:pickValue(x);
}
function pickDisplay90(x){
  if(!x||x.type!=='pick'||typeof window.draftPickProjection90!=='function')return priorAssetLabel90(x);
  const p=window.draftPickProjection90(x)||{},round=Number(x.round)||1,slot=Number(p.projectedSlot)||16,projected=`${round}.${String(slot).padStart(2,'0')}`,original=p.originalTeam||`Roster ${p.originalRoster||'?'}`,current=p.currentOwnerTeam||`Roster ${p.currentOwner||x.owner||'?'}`,value=canonicalPickDisplayValue90(x,p);
