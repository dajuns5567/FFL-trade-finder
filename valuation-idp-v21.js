...
window.offenseScoringFoundationAudit=function(nameOrId){
 ...
 return{id,name:playerName(id),pos,ppg:Number((rs.ppg||0).toFixed(3)),historicalSeasons:rs.historicalSeasons,qualifyingSeasons:rs.seasons,confidence:Number((rs.confidence||0).toFixed(3)),weightCoverage:Number((rs.weightCoverage||0).toFixed(3)),calculationWeight:Number(calc.toFixed(3)),plannedWeight:Number(planned.toFixed(3)),currentAssignedWeight:Number(current.toFixed(3)),currentPlannedShare:planned>0?Number((100*current/planned).toFixed(1)):0,currentEffectiveShare:calc>0?Number((100*current/calc).toFixed(1)):0,weightAmplification:current>0&&planned>0&&calc>0?Number(((current/calc)/(current/planned)).toFixed(3)):1,seasons:(rs.samples||[]).map(s=>({season:s.season,currentSeason:!!s.currentSeason,games:s.games,points:Number((s.points||0).toFixed(2)),ppg:Number((s.ppg||0).toFixed(3)),assignedWeight:Number((s.assignedWeight||0).toFixed(3))}))};
};
