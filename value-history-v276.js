...
function similarPlayersSection(id){
  ...
  const group=(above,below)=>`<div class="vh-neighbor-list">${above.map(row).join('')}${above.length&&below.length?'<div class="tiny muted" style="padding:4px 0">Selected player</div>':''}${below.map(row).join('')}</div>`;
