...
window.openValueHistoryLeagueNetModalV476=function(){const btn=document.querySelector('.tabs button[data-tab="valueHistory"]');btn?.click();currentView='team';syncSubnav();const ids=leagueTeamIds();if(!trackedTeamId&&ids.length)trackedTeamId=String(ids[0]);const go=()=>{if(marketCache){openTeamNetModal();return}loadMarket().then(()=>openTeamNetModal()).catch(()=>{})};setTimeout(go,80)};
